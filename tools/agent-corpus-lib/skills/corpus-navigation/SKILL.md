...
description: How to slice the Bus Priority Impact Studio data corpus from inside the bp-sandbox container — bp_corpus API, mount layout, example bash/python sequences, and determinism rules that load-bearing for cited code_execution evidence refs.
---

# Corpus navigation

The findings-propose agent runs inside a Docker sandbox with the corpus mounted
read-only. The agent has two tools — `bash_exec(code)` and `python_exec(code)`
— and a helper library `bp_corpus` available at `import bp_corpus`. Outputs
(stdout) are captured per call and the agent decides what to do next.
