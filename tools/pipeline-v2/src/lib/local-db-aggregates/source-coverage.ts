...
export type EvidenceRole = "primary" | "context" | "caveat" | "missing_data" | "coverage_audit";
...
const DECISIONS: readonly SourceDecision[] = [
  "complete_for_history",
  "backfill_required",
  "release_context_only",
  "current_signal_only",
...
export const SOURCE_COVERAGE_CONFIGS: readonly SourceConfig[] = [
  {
...
function emptyDetectorEligibilityCounts(): Record<DetectorEligibility, number> {
  const eligibilities: readonly DetectorEligibility[] = [
    "automatic_primary",
    "manual_review_primary",
    "context_only",
    "current_signal_only",
  ...
  return Object.fromEntries(eligibilities.map((eligibility) => [eligibility, 0])) as Record<
    DetectorEligibility,
    number
  >;
}
...
export function buildSourceCoverageLedger(input: {
  ...
  for (const config of input.sourceConfigs ?? SOURCE_COVERAGE_CONFIGS) {
    if (!tableExists(input.sqlite, config.tableName)) {
      const decision = config.forceDecision ?? "backfill_required";
      const readiness: SourceCoverageLedgerEntry["readiness"] = {
