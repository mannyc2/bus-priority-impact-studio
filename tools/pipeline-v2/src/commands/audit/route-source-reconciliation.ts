...
import { canonicalRouteId, normalizeRouteIdText } from "../../lib/route-ids.ts";
...
function canonicalSet(rawRoutes: ReadonlySet<string>, catalogRoutes: ReadonlySet<string>): Set<string> {
  return new Set(
    [...rawRoutes]
...
function sourceYearsForHistoryWindow(startMonth: string, endMonth: string): number[] {
  ...
  return [
    ...new Set(monthRange(start.year, start.month, end.year, end.month).map((month) => month.year)),
...
export function buildRouteSourceReconciliation(input: {
  ...
  ].sort((left, right) => left.source.localeCompare(right.source) || left.rawRouteId.localeCompare(right.rawRouteId));
