...
import { detectorEvaluationArtifactPath } from "../evaluate/detectors.ts";
import { analyticsDetectorReadinessPath } from "./analytics-detector-readiness.ts";
import { dataProductCompletenessPath } from "./data-product-completeness.ts";
...
export type BuildAnalysisDependencyClosureInput = {
  ...
  readonly manifest?: DataProductManifest;
  readonly releaseMonth: string;
...
function productDependency(input: {
  ...
  readonly required: boolean;
  readonly productsById: ReadonlyMap<string, DataProduct>;
  ...
  async run({ input }) {
    ...
    const detectorReadinessInputPath =
      input.options.detectorReadiness === undefined
        ? analyticsDetectorReadinessPath(artifactRoot, historyStartMonth, releaseMonth)
