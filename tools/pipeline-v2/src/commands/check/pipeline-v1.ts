...
type PipelineV1Issue = { code: string; message: string };
...
export type PipelineV1CheckResult = {
  isoMonth: string;
  status: CheckStatus;
  issueCount: number;
...
function collectionWindowSeconds(input: {
  ...
  if (elapsedSeconds === null) return Math.max(0, input.requestedDurationSeconds);
...
async function sourceProbeFreshnessRows(input: {
  ...
  for (const sourceId of requiredV1SourceProbeIds) {
    ...
    const parsed = (await file.json()) as { checkedAt?: unknown; probeStatus?: unknown };
    ...
    const status =
      probeStatus !== "active" || probeAgeDays === null
        ...
        : probeAgeDays > input.maxAgeDays
          ? "stale"
          : "fresh";
    rows.push({ sourceId, status, checkedAt, ageDays: probeAgeDays, probeStatus });
  }
  return rows;
}

export type CheckPipelineV1Inputs = {
  ...
  maxCorridorAmbiguousRouteShare?: number | undefined;
  maxCorridorUnassignedRouteShare?: number | undefined;
  maxSourceProbeAgeDays?: number | undefined;
...
export async function runCheckPipelineV1(
  inputs: CheckPipelineV1Inputs,
  ...
  const minObservedRouteShare = clampShare(
    inputs.minObservedRouteShare ?? defaultMinObservedRouteShare,
  );
  ...
  const minObservedRouteCountOverride =
    inputs.minObservedRouteCount === undefined
      ? 0
      : Math.max(0, Math.round(inputs.minObservedRouteCount));
  const maxCorridorAmbiguousRouteShare = clampShare(
    inputs.maxCorridorAmbiguousRouteShare ?? defaultMaxCorridorAmbiguousRouteShare,
  );
  const maxCorridorUnassignedRouteShare = clampShare(
    inputs.maxCorridorUnassignedRouteShare ?? defaultMaxCorridorUnassignedRouteShare,
  );
  const maxSourceProbeAgeDays = Math.max(
    1,
    Math.round(inputs.maxSourceProbeAgeDays ?? defaultMaxSourceProbeAgeDays),
  );
  const artifactRoot = inputs.artifactRoot ?? defaultArtifactRootPath();
  const sourceFreshness = await sourceProbeFreshnessRows({
    metadataDir: sourceMetadataDir(inputs.sourceMetadataDir),
  ...
  const publicRouteIds = routeBriefs.filter((row) => row.publicVisible).map((row) => row.routeId);
  const buildEligibleRouteIds = readiness
  ...
  const observedReliabilityObservedRows = observedReliability.filter(
  ...
  const observedReliabilityInsufficientRows = observedReliability.filter(
  ...
  for (const row of observedReliability) {
  ...
  const observedReliabilityBelowThresholdRows = observedReliability.filter(
    (row) => row.reliabilityStatus === "observed" && row.sampleCount < row.minSampleThreshold,
  ).length;
  const observedReliabilityHeadwaySampleCount = observedReliability.reduce(
  ...
  const allObservedReliabilityRunIds = unique(observedReliability.map((row) => row.runId));

  const collectionRunsAll = await listGtfsRtCollectionRuns(local.db);
  const collectionRuns = collectionRunsAll.filter((row) =>
  ...
  const gtfsRtState = { collectionRuns, feedSnapshots, parsedSnapshots, observedHeadwaySamples };
  ...
  const evaluatedInterventionComparisonRows = interventionComparisons.filter(
    (row) => row.comparisonStatus === "evaluated",
  ).length;
  const evaluatedInterventionComparisonRidershipDeltaRows = interventionComparisons.filter(
    (row) => row.comparisonStatus === "evaluated" && row.ridershipDelta !== null,
  ).length;
  const peerAdjustedInterventionComparisonRows = interventionComparisons.filter(
    (row) =>
  ...
  const busLaneMatchedPublicRouteIds = routeBriefs
  ...
  const routeMonthTrendSpeedRows = routeMonthTrends.filter((row) => row.hasSpeedTrend).length;
  const routeMonthTrendRidershipRows = routeMonthTrends.filter(
    (row) => row.hasRidershipTrend,
  ).length;
  const corridorAssignedRouteMemberRows = corridorMembers.filter(
    (row) => row.assignmentStatus === "assigned",
  ).length;
  const corridorAmbiguousRouteMemberRows = corridorMembers.filter(
    (row) => row.assignmentStatus === "ambiguous",
  ).length;
  const corridorUnassignedRouteMemberRows = corridorMembers.filter(
    (row) => row.assignmentStatus === "unassigned",
  ).length;
  const corridorSegmentEvidenceRouteMemberRows = corridorMembers.filter(
  ...
  if (coverage.length < publicRouteIds.length) {
    addIssue(
      issues,
      "route_coverage_incomplete",
      `Route/month coverage has ${coverage.length} rows for ${publicRouteIds.length} public routes.`,
    );
  }
  if (buildEligibleRouteIds.length === 0) {
    addIssue(issues, "build_eligible_routes_missing", "No build-eligible route rows exist.");
  }
  if (batchStatus?.status !== "pass") {
    addIssue(
      issues,
      "route_batch_status_not_pass",
      `Route batch status is ${batchStatus?.status ?? "missing"}.`,
    );
  }
  if (builtRoutes.length < buildEligibleRouteIds.length) {
    addIssue(
      issues,
      "built_route_count_incomplete",
      `Only ${builtRoutes.length} built routes for ${buildEligibleRouteIds.length} build-eligible routes.`,
    );
  }
  ...
  if (!inputs.allowInsufficientGtfsRt && observedReliabilityObservedRouteCount === 0) {
    addIssue(
      issues,
  ...
  if (
    !inputs.allowInsufficientGtfsRt &&
  ...
  if (!inputs.allowInsufficientGtfsRt && observedReliabilityBelowThresholdRows > 0) {
    addIssue(
      issues,
  ...
  if (!inputs.allowInsufficientGtfsRt && missingGtfsRtRunIds.length > 0) {
    addIssue(
      issues,
  ...
  if (aceRoutes.length > 0 && interventionEvents.length === 0) {
    addIssue(
      issues,
      "intervention_events_missing",
      `${aceRoutes.length} ACE/ABLE source rows exist but no intervention events were generated.`,
    );
  }
  if (aceRoutes.length > 0 && interventionComparisons.length === 0) {
    addIssue(
      issues,
      "intervention_comparisons_missing",
      `${aceRoutes.length} ACE/ABLE source rows exist but no route intervention comparisons were generated.`,
    );
  }
  if (publicRouteIds.length > 0 && routeMonthTrends.length === 0) {
  ...
  if (aceRoutes.length > 0 && evaluatedInterventionComparisonRows === 0) {
    addIssue(
      issues,
  ...
  const comparisonsWithoutCaveats = interventionComparisons.filter(
  ...
  if (interventionComparisons.length > 0 && corridorInterventionContexts.length === 0) {
    addIssue(
      issues,
  ...
  const interventionRouteIds = new Set(interventionComparisons.map((row) => row.routeId));
  ...
  try {
    const d1Result = await runVerifyD1Export({
      local,
      year: inputs.year,
      month: inputs.month,
    ...
    if (aceRoutes.length > 0 && d1.routeInterventionComparisonRows === 0) {
      addIssue(
        issues,
    ...
    if (corridorInterventionContexts.length > 0 && d1.corridorInterventionContextRows === 0) {
      addIssue(
        issues,
    ...
    if (routeMonthTrends.length > 0 && d1.routeMonthTrendRows === 0) {
  ...
  return {
    ...
    issues,
    counts: {
      routeCatalogRows: catalog.length,
      routeCoverageRows: coverage.length,
      routeReadinessRows: readiness.length,
      ...
      routeObservedReliabilityRows: observedReliability.length,
      ...
      aceRouteRows: aceRoutes.length,
      interventionEventRows: interventionEvents.length,
      routeInterventionComparisonRows: interventionComparisons.length,
      ...
      corridorRows: corridors.length,
      corridorRouteMemberRows: corridorMembers.length,
      ...
      corridorInterventionContextRows: corridorInterventionContexts.length,
...
// Helper for callers (audit, finalize) that need to open a separate DB connection
// for a clean rebuild check.
...
export default defineCommand({
  path: ["check", "pipeline-v1"],
  summary: "Run the pipeline v1 release-spine gate against the local pipeline DB.",
  input: {
    options: dbOptions.extend({
      year: arg.positiveInt().default(2026).describe("Calendar year"),
      month: arg.positiveInt().default(3).describe("Calendar month, 1-12"),
      allowInsufficientGtfsRt: z
        .boolean()
        .optional()
        .describe("Skip strict GTFS-RT readiness gates"),
      ...
      maxCorridorAmbiguousRouteShare: z.coerce.number().optional(),
      maxCorridorUnassignedRouteShare: z.coerce.number().optional(),
      maxSourceProbeAgeDays: arg.positiveInt().optional(),
