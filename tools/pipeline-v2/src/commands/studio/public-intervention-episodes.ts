...
import { dbOptions, openLocalPipelineDb } from "../../lib/local-db.ts";
...
export default defineCommand({
  ...
  async run({ input }) {
    ...
    const opened = await openLocalPipelineDb(input.options.db, { readonly: true });
    try {
      const [
        occurrences,
        routeEvidence,
        ...
        listInterventionEvents(opened.db),
