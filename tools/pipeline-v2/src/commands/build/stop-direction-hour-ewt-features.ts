...
function selectScheduleSource(input: {
  ...
  month: string;
  routeId: string;
  requestedSource: "auto" | ScheduleSourceKind;
  gtfsRunId: string | null;
}): { selection: ScheduleSourceSelection; rows: ScheduleStopArrivalForStopDirectionHourEwt[] } {
  const tryGtfs = () => {
...
export function buildStopDirectionHourEwtFeatureArtifactFromDb(input: {
  ...
  routeId: string;
  runId: string;
  scheduleSource: "auto" | ScheduleSourceKind;
  gtfsRunId: string | null;
  timezone: string;
  generatedAt: string;
  dbPath: string | null;
