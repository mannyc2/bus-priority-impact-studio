...
export default defineCommand({
  ...
  async run({ input }) {
    ...
    return runLocalDbCommandBoundary({
      ...
      command: "build.route-month-speed-golden-diff",
      operation: "buildRouteMonthSpeedGoldenDiff",
      run: async (local) => {
        const projected = local.sqlite
          .query(
        ...
        const projectedByKey = new Map(
          projected.map((row) => [`${row.route_id}|${row.month}`, row]),
        );
        ...
        for (const trendRow of trend) {
          ...
          const fields: readonly [Mismatch["field"], number | null, number | null][] = [
            ...
            [
              "averageSpeedMph",
              trendRow.average_speed_mph,
              roundSpeed(projectedRow.average_speed_mph),
            ],
          ];
