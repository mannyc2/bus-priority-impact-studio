 ...
 */
import { createWriteStream, existsSync, readdirSync, readFileSync, mkdirSync } from "node:fs";
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--run-dir") out.runDir = argv[++i];
      ...
      console.log("usage: bun normalize-agentic-payloads.ts [--run-dir D] [--out D] [--limit N] [--report-only] [--examples N]");
      process.exit(0);
    }
  }
  return out;
...
const snake = (s: unknown): string =>
  lc(s).replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
...
const CONCEPT_FIELDS: Record<string, string[]> = {
  ...
  date: ["date", "eventdate", "dateobserved", "datedetail", "datelabel", "datedescription", "datetext"],
  period: ["period", "beforeperiod", "afterperiod", "timeperiod", "metricperiod"],
  geography: ["geography", "geographies", "metricgeography", "corridor", "corridortext", "affectedcorridors", "eventaffectedcorridors"],
...
const ENUM_CONCEPTS = new Set([
  "status", "authority", "truthstatus", "priority", "unit", "direction",
  "metricvaluekind", "claimkind", "questionkind", "contextkind", "entitykind",
  "eventfamily", "tablekind", "documentmode", "modehint", "truthstatus", "wordinggate",
  "treatmentkind",
]);

function conceptFor(fieldName: string): string | null {
  const base = lc(fieldName).replace(/raw$/i, "").replace(/[^a-z0-9]/g, "");
...
const STRUCTURED_PASSTHROUGH = new Set([
  "metricvalue", "metricvaluenumeric", "value", "valuenumeric", "comparisonvalue", "comparisonvaluenumeric",
  "changevalue", "changenumeric", "change", "rowcount", "columncount", "pagenumber", "sourcepagenumbers",
  "routetext", "routeids", "affectedrouteids", "subjectrouteids", "entitycandidateids", "attendeecount",
  "researchusetags", "contenttypes", "attributes", "affectedentities", "rows", "headers", "headertexts",
  "agendaitems", "agencies", "affectedstreets", "affectedintersections",
  // already-typed entity/source metadata (not a normalization gap)
  "sourceid", "sourcegroup", "documentdate", "entityid", "entity", "entitymode", "entitymodehint",
  "entitynamenormalized", "normalizedrefhint", "subjectroute", "subjectentity",
]);
function classifyUnknown(fieldName: string): "structured" | "freetext" | "unknown" {
  ...
  if (/(text|label|name|question|summary|description|reason|note|notes|title|excerpt|quote|comment|detail|details|subject|treatment|comparison|context)$/.test(base)) return "freetext";
  return "unknown";
}
...
const ok = (value: unknown, extra?: Record<string, unknown>): Norm => ({ value, canonical: true, extra });
const fall = (value: unknown, extra?: Record<string, unknown>): Norm => ({ value, canonical: false, extra });

/** General project/intervention status (superset of the domain event-status enum). */
const STATUS_CANON = ["proposed", "planned", "scheduled", "approved", "in_progress", "ongoing", "completed", "implemented", "existing", "cancelled", "unknown"] as const;
function normStatus(v: string): Norm {
  const t = lc(v);
  if (includesAny(t, ["cancel", "withdrawn", "abandoned"])) return ok("cancelled");
  if (includesAny(t, ["implemented", "launched", "in service", "in effect", "operational"])) return ok("implemented");
  if (includesAny(t, ["complete", "completed", "done", "finished", "delivered"])) return ok("completed");
  if (includesAny(t, ["in_progress", "in progress", "underway", "construction", "active"])) return ok("in_progress");
  ...
  if (t === "" || t === "unknown" || t === "n/a" || t === "tbd" || t === "not specified") return ok("unknown");
  return fall(snake(v));
}
...
function normEventStatus(v: string): Norm {
  const t = lc(v);
  if (includesAny(t, ["implemented", "launched", "complete", "in service", "in effect", "operational"])) return ok("implemented");
  if (includesAny(t, ["approved", "adopted"])) return ok("approved");
...
function normAuthority(v: string): Norm {
  ...
  else if (includesAny(t, ["dca", "city council", "community board", "cb"])) agency = "nyc_local_gov";
  else if (includesAny(t, ["consultant", "aecom", "sam schwartz", "wsp", "hdr"])) agency = "consultant";
  else if (includesAny(t, ["advocacy", "riders alliance", "tstc", "transportation alternatives"])) agency = "advocacy";

  let metricAuthority: string;
  if (includesAny(t, ["customer journey", "abst", "additional bus stop time", "customer"])) metricAuthority = "official_customer_metric";
  else if (includesAny(t, ["comptroller", "audit", "independent"])) metricAuthority = "independent_audit";
  else if (includesAny(t, ["mta", "nyct", "dot", "agency", "official"])) metricAuthority = "official_agency_metric";
...
const UNIT_ALIAS: Record<string, string> = {
  "%": "percent", pct: "percent", percentage: "percent", percent: "percent", percentages: "percent",
  ...
  rider: "riders", riders: "riders", ridership: "riders",
  passenger: "passengers", passengers: "passengers",
...
function normDirection(v: string): Norm {
  const t = lc(v);
  const compass =
    includesAny(t, ["north", "nb", "uptown"]) ? "NB" :
    includesAny(t, ["south", "sb", "downtown"]) ? "SB" :
    includesAny(t, ["east", "eb"]) ? "EB" :
    includesAny(t, ["west", "wb"]) ? "WB" :
    includesAny(t, ["both", "bidirectional", "two-way", "two way"]) ? "both" : null;
  const change =
    includesAny(t, ["increase", "faster", "higher", "up", "improv", "gain"]) ? "increase" :
    includesAny(t, ["decrease", "slower", "lower", "down", "reduc", "loss", "drop"]) ? "decrease" :
    includesAny(t, ["no change", "none", "flat", "unchanged"]) ? "none" : null;
...
const normMetricValueKind = makeEnum(
  ["absolute", "percent", "textual", "range", "duration", "currency", "rate", "count", "ratio", "unknown"],
  { textual_qualitative: "textual", qualitative: "textual", monetary: "currency", percentage: "percent", time: "duration" },
);
const normClaimKind = makeEnum(
  ["existing_condition", "proposed_treatment", "project_scope", "public_feedback", "performance_observation", "methodology_note", "policy_or_operations_statement", "problem_statement", "other"],
  { existing_conditions: "existing_condition", feedback: "public_feedback", performance: "performance_observation", methodology: "methodology_note", scope: "project_scope", problem: "problem_statement" },
);
const normQuestionKind = makeEnum(
  ["missing_detail", "content_gap", "design_detail", "data_gap", "clarification", "missing_context", "scope_clarification", "content_anticipation", "other"],
  { missing_information: "missing_detail", data: "data_gap", clarify: "clarification" },
);
const normContextKind = makeEnum(
  ["section_heading", "presentation_context", "document_context", "project_timeline", "document_structure", "meeting_context", "page_context", "other"],
  { section_context: "section_heading", heading: "section_heading", presentation: "presentation_context", meeting: "meeting_context" },
);
const normEventFamily = makeEnum(
  ["community_engagement", "implementation", "planning", "design", "service_change", "milestone", "study", "construction", "approval", "other"],
  { community_outreach: "community_engagement", public_outreach: "community_engagement", community_outreach_meeting: "community_engagement", outreach: "community_engagement", project_phase: "milestone", project_milestone: "milestone", service_launch: "service_change", service_change_candidate: "service_change" },
);
const normTableKind = makeEnum(
  ["legend", "comparison_table", "breakdown", "stop_list", "lane_configuration", "schedule", "mode_share", "other"],
  { map_legend: "legend", legend_table: "legend", percentage_breakdown: "breakdown", mode_share_table: "mode_share", cross_section_lane_configuration: "lane_configuration" },
);
...
function normDocumentModeWrapped(v: string): Norm {
  const s = snake(v);
  if (s.startsWith("presentation_slide") || s.startsWith("presentation")) return ok("presentation_slide");
  return normDocumentMode(v);
}
const normModeHint = makeEnum(
  ["bus", "rail", "subway", "select_bus_service", "roadway", "ferry", "bike", "pedestrian", "unknown"],
  { sbs: "select_bus_service", brt: "select_bus_service", bus_rapid_transit: "select_bus_service", local_bus: "bus", limited_bus: "bus", train: "rail", metro: "subway", walk: "pedestrian", bicycle: "bike" },
);
const normTreatmentKind = makeEnum(
  ["bus_lane", "busway", "offset_bus_lane", "transit_signal_priority", "camera_enforcement", "stop_optimization", "all_door_boarding", "fare_payment", "roadway_redesign", "signal_timing", "pavement_markings", "pedestrian_safety", "parking_changes", "turn_restriction", "other"],
  { tsp: "transit_signal_priority", signal_priority: "transit_signal_priority", ace: "camera_enforcement", able: "camera_enforcement", bus_lane_camera: "camera_enforcement", red_lane: "bus_lane", bus_lanes: "bus_lane", offset_lane: "offset_bus_lane", stop_consolidation: "stop_optimization", stop_changes: "stop_optimization", boarding: "all_door_boarding", off_board_fare: "fare_payment", roadway: "roadway_redesign", street_redesign: "roadway_redesign", markings: "pavement_markings" },
);
...
function normEntityKind(v: string): Norm {
  ...
  if (includesAny(t, ["lane", "busway", "camera", "tsp", "treatment", "design"])) return ok("treatment");
...
function normTruthStatus(v: string): Norm {
  ...
  if (includesAny(t, ["deterministic", "computed", "measured"])) return ok("deterministic_project_metric");
  if (includesAny(t, ["stated", "proposed", "plausible", "claim", "source_statement", "source_stated", "needs_review", "design"])) return ok("document_claim_only");
...
function classifyDatePrecision(text: string): string {
  if (text.includes(" to ") || text.includes(" through ") || /\d{4}\s*[-/]\s*\d{4}/.test(text)) return "range";
  if (/\b\d{4}-\d{2}-\d{2}\b/.test(text)) return "day";
  if (/\b(jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s+\d{1,2},?\s+\d{4}\b/.test(text)) return "day";
  if (/\b(jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s+\d{4}\b/.test(text)) return "month";
...
function normDate(v: string): Norm {
  ...
  if ((m = t.match(/\b(\d{4})-(\d{2})-(\d{2})\b/))) iso = `${m[1]}-${m[2]}-${m[3]}`;
  else if ((m = t.match(/\b(jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s+(\d{1,2}),?\s+(\d{4})\b/))) iso = `${m[3]}-${MONTHS[m[1]]}-${String(m[2]).padStart(2, "0")}`;
  else if ((m = t.match(/\b(jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s+(\d{4})\b/))) iso = `${m[2]}-${MONTHS[m[1]]}`;
  else if ((m = t.match(/\b(\d{4})\b/))) iso = m[1];
...
const STREET_ABBR: Array<[RegExp, string]> = [
  [/\bst\b\.?/g, "street"], [/\bave\b\.?/g, "avenue"], [/\bav\b\.?/g, "avenue"],
  [/\bblvd\b\.?/g, "boulevard"], [/\brd\b\.?/g, "road"], [/\bpkwy\b\.?/g, "parkway"],
  [/\bpl\b\.?/g, "place"], [/\bdr\b\.?/g, "drive"], [/\bln\b\.?/g, "lane"], [/\bhwy\b\.?/g, "highway"],
];
const BOROUGHS = ["manhattan", "brooklyn", "queens", "bronx", "staten island"];
function normPlace(v: string): Norm {
  let t = lc(v);
  if (t === "" || t === "unknown" || t === "not specified" || t === "new york city" || t === "nyc" || t === "citywide") {
  ...
  t = t.replace(/\b(corridor|area|study area|project area)\b/g, "").replace(/,\s*(manhattan|brooklyn|queens|bronx|staten island)\b/g, "");
...
function normalizeConcept(concept: string, value: unknown, surfaceKind: string): Norm {
  ...
  switch (concept) {
    case "status": return surfaceKind === "event_candidate" || surfaceKind === "service_change_candidate" ? normEventStatus(v) : normStatus(v);
    case "authority": return normAuthority(v);
    case "truthstatus": return normTruthStatus(v);
    case "wordinggate": return normWordingGate(v);
    case "priority": return PRIORITY_BY(v);
    case "unit": return normUnit(v);
    case "direction": return normDirection(v);
    case "metricvaluekind": return normMetricValueKind(v);
    case "claimkind": return normClaimKind(v);
    case "questionkind": return normQuestionKind(v);
    case "contextkind": return normContextKind(v);
    case "entitykind": return normEntityKind(v);
    case "eventfamily": return normEventFamily(v);
    case "tablekind": return normTableKind(v);
    case "documentmode": return normDocumentModeWrapped(v);
    case "modehint": return normModeHint(v);
    case "treatmentkind": return normTreatmentKind(v);
    case "date": return normDate(v);
    case "period": return normDate(v); // periods reuse the date parser (scenario labels fall through to precision=unknown)
    case "geography":
    case "location": return normPlace(v);
    case "eventsubtype":
    case "role":
    default: return fall(snake(v)); // recognized-but-free-text concept
  }
}

// canonical domain surfaceKind for each agentic surfaceKind
const SURFACE_KIND_CANON: Record<string, string> = {
  entity_mention: "entity", metric_observation: "metric_claim", event_candidate: "event",
  table_observation: "table", claim: "claim", context_signal: "context_signal",
  review_question: "review_question", relation: "relation",
  treatment_component: "entity", source_note: "context_signal",
  service_change_candidate: "event", causal_claim: "claim",
  finding_reasoning_seed: "claim", source_gap_seed: "review_question",
};
...
type ConceptStat = { instances: number; canonical: number; fallthrough: number; unmappedTop: Map<string, number> };
const conceptStats: Record<string, ConceptStat> = {};
const rawFieldStats = { total: 0, mappedToConcept: 0, structuredPassthrough: 0, freetextPassthrough: 0, trulyUnknown: 0, unknownTop: new Map<string, number>() };
let drafts = 0, draftsWritten = 0;
const examples: unknown[] = [];

function bump(concept: string, n: Norm, rawValue: unknown) {
  const s = (conceptStats[concept] ??= { instances: 0, canonical: 0, fallthrough: 0, unmappedTop: new Map() });
  s.instances++;
  if (n.canonical) s.canonical++;
  else {
    s.fallthrough++;
    if (ENUM_CONCEPTS.has(concept)) {
      const key = compact(Array.isArray(rawValue) ? rawValue.join("|") : rawValue).slice(0, 60) || "(empty)";
      s.unmappedTop.set(key, (s.unmappedTop.get(key) ?? 0) + 1);
    }
  }
}

const shards = readdirSync(shardRoot).filter((d) => d.startsWith("shard-")).sort();
outer: for (const sh of shards) {
  const sdir = join(shardRoot, sh);
  let wins: import("node:fs").Dirent[];
  try { wins = readdirSync(sdir, { withFileTypes: true }).filter((e) => e.isDirectory() && !e.name.startsWith(".")); }
  catch { continue; }
  for (const w of wins) {
    const ap = join(sdir, w.name, "artifact.json");
    if (!existsSync(ap)) continue;
    let art: any;
    try { art = JSON.parse(readFileSync(ap, "utf8")); } catch { continue; }
    if ((art.summary?.acceptedCount ?? 0) <= 0) continue;
    const sourceId = art.source?.sourceId ?? null;
    const pageNumbers = art.source?.pageNumbers ?? [];
    for (const d of art.drafts ?? []) {
      drafts++;
      const surfaceKind: string = d.surfaceKind ?? "?";
      const payload: Record<string, unknown> = d.rawPayload ?? {};
      ...
      for (const [k, val] of Object.entries(payload)) {
        ...
        if (!(concept in normalized)) {
          normalized[concept] = { value: n.value, canonical: n.canonical, sourceField: k, ...(n.extra ?? {}) };
        }
      }
      if (jsonl) {
        const rec = {
          surfaceId: d.surfaceId ?? null,
          surfaceKind,
          surfaceKindCanonical: SURFACE_KIND_CANON[surfaceKind] ?? "other",
          corpusRole: d.corpusRole ?? null,
          sourceId,
          pageNumbers,
          // routes are already structured upstream — carry them through, don't re-derive
          routeIds: [...new Set((d.canonicalSelections ?? []).flatMap((s: any) => s.selectedIds ?? []))],
          normalized,
          unmappedKeys,
        ...
        draftsWritten++;
        if (examples.length < args.examples) examples.push({ surfaceKind, rawPayload: payload, normalized });
      }
...
const conceptReport: Record<string, unknown> = {};
let enumInstances = 0, enumCanonical = 0;
for (const [c, s] of Object.entries(conceptStats).sort((a, b) => b[1].instances - a[1].instances)) {
  if (ENUM_CONCEPTS.has(c)) { enumInstances += s.instances; enumCanonical += s.canonical; }
  conceptReport[c] = {
    enum: ENUM_CONCEPTS.has(c),
    instances: s.instances,
    canonicalPct: +((100 * s.canonical) / s.instances).toFixed(1),
    fallthrough: s.fallthrough,
    topUnmapped: [...s.unmappedTop.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10).map(([v, n]) => `${v} (${n})`),
  };
}
const report = {
  ...
  rawFields: {
    ...
    topUnknownFields: [...rawFieldStats.unknownTop.entries()].sort((a, b) => b[1] - a[1]).slice(0, 30).map(([f, n]) => `${f} (${n})`),
  },
  enumConcepts: { instances: enumInstances, canonicalPct: enumInstances ? +((100 * enumCanonical) / enumInstances).toFixed(1) : 0 },
...
console.log(JSON.stringify({ ...report, concepts: undefined, _examples: args.reportOnly ? examples.slice(0, args.examples) : undefined }, null, 2));
console.log(`\nconcepts (canonical%):`);
for (const [c, r] of Object.entries(conceptReport)) {
  const rr = r as any;
  console.log(`  ${c.padEnd(16)} ${String(rr.instances).padStart(6)}  ${String(rr.canonicalPct).padStart(5)}%  ${rr.enum ? "enum" : "open"}`);
}
if (!args.reportOnly) console.log(`\nwrote ${draftsWritten} records -> ${jsonlPath}\nreport -> ${reportPath}`);
