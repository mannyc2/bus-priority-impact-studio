// Phase 3 (intervention-records synthesis) unit tests covering the
// post-LLM repair / validation / chunking / dedupe helpers.
...
import type { Tier2CandidateSourceRef } from "../../../../src/commands/docs/tier2/_shared.ts";
...
describe("repairInterventionRecordsAliases (Fix 6)", () => {
  test("rewrites comparisonPeriodStart/End into nested comparisonPeriod", () => {
    ...
    const { patched, repairedRecordIndices } =
      repairInterventionRecordsAliases(toolArgs);
    expect(repairedRecordIndices).toEqual([0]);
    const root = patched as { interventionRecords: Array<{ metrics: Array<Record<string, unknown>> }> };
    const metric = root.interventionRecords[0]!.metrics[0]!;
  ...
  test("demotes invented enum values to custom labels (Fix P1.5, audit follow-up)", async () => {
    const { DocumentInterventionRecordsToolResponseSchema } = await import("@bp/domain");
    const { repairInvalidEnumValues } = await import("../../../../src/commands/docs/tier2/_shared.ts");
    const toolArgs = {
      sourceId: "x",
      interventionRecords: [
        {
    ...
    if (reparse.success) {
      const record = reparse.data.interventionRecords[0]!;
  ...
  test("drops empty corridor and empty extentEndpoints (Fix P1.6)", () => {
    ...
    expect("corridor" in root.interventionRecords[0]!).toBe(false);
    // extentEndpoints with empty start → dropped, streets preserved
    const r1 = root.interventionRecords[1]!["corridor"] as Record<string, unknown>;
    expect("extentEndpoints" in r1).toBe(false);
    expect(r1["streets"]).toEqual(["14th Street"]);
    // Corridor with empty streets → corridor deleted
    expect("corridor" in root.interventionRecords[2]!).toBe(false);
  });
  ...
  test("coerces statusHistory status synonyms (Fix P1.6)", () => {
    ...
    expect(root.interventionRecords[0]!.statusHistory.map((e) => e.status)).toEqual([
      "complete",
      "implementing",
      "implementing",
      "planning",
  ...
  test("strips unrecognized keys from metric entries (Fix P1.7)", async () => {
    ...
    expect(reparse.success).toBe(true);
    if (reparse.success) {
      const metric = reparse.data.interventionRecords[0]!.metrics[0]!;
  ...
  test("drops statusHistory entries with unrecognizable status (P1.6)", async () => {
    ...
    expect(reparse.success).toBe(true);
    if (reparse.success) {
      const statuses = reparse.data.interventionRecords[0]!.statusHistory.map(
        (entry) => entry.status,
      );
  ...
  test("filters null array elements (Fix P1.4 audit follow-up)", () => {
    ...
    expect(root.interventionRecords[0]!.customTreatments).toEqual([
      "frequency_increase",
      "other",
    ]);
  });
  ...
  test("strips null optional fields (Fix P1.4)", () => {
    ...
    const corridor = root.interventionRecords[0]!.corridor;
    expect("extentEndpoints" in corridor).toBe(false);
    expect("serviceMode" in root.interventionRecords[0]!).toBe(false);
  ...
  test("repairs conflicting canonical/custom metric and component labels", () => {
    ...
    if (result.status === "extracted") {
      ...
      expect(record.extraction.qualityRepairs).toContain(
...
describe("validateMetricValueNumericSupport (Fix 3 + P2.4)", () => {
  test("keeps valueNumeric when supported by candidate quote", () => {
    ...
    const candidateById = new Map([["c1", c1]]);
    const { unsupportedValueNumericRemoved, metric } =
      validateMetricValueNumericSupport({
        metric: {
          valueNumeric: 15,
          unit: "percent",
...
describe("normalizeCorridorText + validateCorridorExtentEndpoints (Fix 4)", () => {
  ...
  test("keeps endpoints when both appear in supporting quote (with suffix normalization)", () => {
    const c1 = buildCandidate({
      candidateId: "c1",
      evidenceQuote:
        "The busway runs from 14th Street to 42nd Street on Broadway.",
    });
...
describe("recordHasInterventionEvidence (Fix 7 + P2.5)", () => {
  ...
  test("rejects a record whose only candidate is a fare-policy quote", () => {
    const c1 = buildCandidate({
      candidateType: "document_treatment_component_candidate",
      evidenceQuote:
        "Implement fare capping and OMNY-based fare policy across all customers.",
      fields: {},
    });
  ...
  test("rejects records whose only candidate is a subway-elevator quote", () => {
    const c1 = buildCandidate({
      candidateType: "document_treatment_component_candidate",
      evidenceQuote: "Install new elevators and escalators at the subway station for accessibility.",
      fields: {},
    });
    expect(recordHasInterventionEvidence([c1])).toBe(false);
  });
...
describe("runInterventionRecordsBucket provider errors", () => {
  test("preserves top-level DeepSeek provider errors instead of reporting a missing tool call", async () => {
    ...
    try {
      ...
      expect(errorJson.message).toContain("Insufficient Balance");
    } finally {
...
describe("recordsAreClusterCompatible (Fix P1.3)", () => {
  ...
  test("incompatible when records share only a corridor-distant context", () => {
    ...
    const b = buildRecord({
      routes: ["B44"],
      primaryTreatments: ["transit_signal_priority"] as Tier2DocumentInterventionRecord["primaryTreatments"],
    });
    expect(recordsAreClusterCompatible(a, b)).toBe(false);
  });
});
...
describe("dedupeInterventionRecordsByEvidenceOverlap (Fix 8)", () => {
  test("does NOT merge records that share a candidate but have no other compatibility (Fix P1.3)", () => {
    ...
    const b = buildRecord({
      recordId: "b",
      routes: ["Q23"],
      primaryTreatments: ["off_board_fare_collection"] as Tier2DocumentInterventionRecord["primaryTreatments"],
      evidenceCandidateIds: ["c1"],
    });
  ...
  test("merges records that share a candidate plus route and treatment", () => {
    ...
    expect(out[0]?.extraction.bucketId).toBe(
      "test:per_route:M15,test:per_route:Q23",
    );
  });
});
...
describe("mergeRecordCluster (Fix P2.6 — evidence refs union per component)", () => {
  test("unions evidenceRefs across duplicate treatmentComponents", () => {
    ...
    const a = buildRecord({
      ...
      treatmentComponents: [
        {
          treatmentType: "bus_lane" as DocumentInterventionRecord["treatmentComponents"][number]["treatmentType"],
