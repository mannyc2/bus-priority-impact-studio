...
import {
  ...
  loadAnalyticsCorpusObservations,
} from "../../../src/commands/audit/analytics-corpus-profile.ts";
...
describe("analytics corpus profile audit", () => {
  test("loads route/month observations and summarizes historical readiness", () => {
    ...
    try {
      ...
      const observations = loadAnalyticsCorpusObservations(sqlite);
      expect(observations.some((observation) => observation.sourceId === "route_month_trends_speed"))
        .toBe(true);
      expect(
