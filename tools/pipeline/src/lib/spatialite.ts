...
const CANDIDATE_PATHS = [
  // Explicit override wins. The override is passed verbatim (with whatever
  // suffix the user provides).
  ...
  // Last resort: lookup via the default linker.
  "mod_spatialite",
...
/**
 * Load mod_spatialite into the given Database. Idempotent within a single
 * connection — re-calling on the same handle is harmless because we wrap the
 * call in a try/catch over "already initialized" cases.
 *
 * Throws SpatialiteUnavailableError with an actionable message if no candidate
 * path is loadable.
 */
...
/**
 * Spatialite needs `spatial_ref_sys` and related metadata tables before
 * geometry columns or spatial indexes can be created. `InitSpatialMetaData(1)`
 * is the fast initializer; it is idempotent and cheap on already-initialized
 * databases.
 */
