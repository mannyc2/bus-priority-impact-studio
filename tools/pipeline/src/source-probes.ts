...
import type { SocrataMetadata } from "@bp/sources";
import {
...
import type { ManifestSource, SocrataManifestSource } from "./source-manifest.js";
...
export function summarizeProbeOutputs(outputs: SourceProbeOutput[]): {
  schemaVersion: typeof probeSchemaVersion;
  checkedAt: string;
  sourceCount: number;
  activeCount: number;
  ...
  const activeCount = outputs.filter((output) => output.probeStatus === "active").length;
  ...
  const checkedAt = outputs[0]?.checkedAt ?? new Date().toISOString();
  ...
  return {
    ...
    sources: outputs.map((output) => {
      const summary = {
        ...
        priority: output.sourcePriority,
        manifestStatus: output.manifestStatus,
        probeStatus: output.probeStatus,
        statusRecommendation: output.statusRecommendation,
      };

      if (output.error !== undefined) {
        return { ...summary, error: output.error };
      }
