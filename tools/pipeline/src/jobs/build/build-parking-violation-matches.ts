...
import { type OpenLocalPipelineDb, withLocalPipelineDb } from "../../lib/local-db.js";
import {
  numericHouseNumber,
  parkingLocationKey,
...
type Args = {
  ...
  artifactRoot?: string;
  output?: string;
  hydrateRawFields?: boolean;
  rawParkingDir?: string;
  rawLionPath?: string;
  skipGeoclient?: boolean;
  maxCameraGroups?: number;
  maxAddressGroups?: number;
  computedAt?: Date;
...
function parseCliArgs(args: string[]): Args {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--db-path" && value !== undefined) {
      out.dbPath = fromCliPath(value);
      index += 1;
    } else if (arg === "--artifact-root" && value !== undefined) {
      out.artifactRoot = fromCliPath(value);
      index += 1;
    } else if (arg === "--output" && value !== undefined) {
      out.output = fromCliPath(value);
      index += 1;
    } else if (arg === "--raw-parking-dir" && value !== undefined) {
      out.rawParkingDir = fromCliPath(value);
      index += 1;
    } else if (arg === "--raw-lion" && value !== undefined) {
      out.rawLionPath = fromCliPath(value);
      index += 1;
    } else if (arg === "--max-camera-groups" && value !== undefined) {
      out.maxCameraGroups = Number(value);
      index += 1;
    } else if (arg === "--max-address-groups" && value !== undefined) {
      out.maxAddressGroups = Number(value);
      index += 1;
    } else if (arg === "--hydrate-raw-fields") {
      out.hydrateRawFields = true;
    } else if (arg === "--skip-geoclient") {
      out.skipGeoclient = true;
...
export async function buildParkingViolationMatches(args: Args = {}): Promise<Result> {
  const artifactRoot = args.artifactRoot ?? defaultArtifactRootPath();
  const auditArtifactPath = args.output ?? parkingViolationMatchAuditPath(artifactRoot);
  const computedAt = (args.computedAt ?? new Date()).toISOString();

  const result = await withLocalPipelineDb(
    args.dbPath,
    async (local) => {
      ...
      if (args.auditOnly === true) {
        ...
        if (args.hydrateRawFields === true) {
          hydratedLionRows = await hydrateLionRawFields(local, args.rawLionPath);
          hydratedParkingRows = await hydrateParkingRawFields(local, args.rawParkingDir);
        }
        ...
        refreshedLocationKeyRows =
          args.hydrateRawFields === true || existingLocationKeys > 0
            ? 0
            : refreshParkingLocationKeys(local);
        local.sqlite.exec("DELETE FROM local_parking_violation_match");

        const geoclient =
          args.skipGeoclient === true
            ? null
            : args.geoclient === undefined
        ...
        for (const group of listCameraGroups(local, args.maxCameraGroups)) {
        ...
        for (const group of listAddressGroups(local, args.maxAddressGroups)) {
...
export async function buildParkingViolationMatchesFromCli(args: string[]): Promise<Result> {
  const result = await buildParkingViolationMatches(parseCliArgs(args));
  console.log(
    `parking-violation-matches: hydrated_parking=${result.hydratedParkingRows} hydrated_lion=${result.hydratedLionRows} keys=${result.refreshedLocationKeyRows} camera_groups=${result.cameraGroupsScanned} address_groups=${result.addressGroupsScanned} matches=${result.matchRows} groups=${result.matchedLocationGroups} represented_events=${result.representedEvents} routes=${result.routeCount} audit=${result.auditArtifactPath}`,
  );
  return result;
}
