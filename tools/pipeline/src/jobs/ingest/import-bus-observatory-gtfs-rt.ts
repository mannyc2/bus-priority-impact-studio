...
type ImportBusObservatoryGtfsRtArgs = DbArgs & {
  runId?: string;
  year?: number;
  month?: number;
  canonicalCsv?: string;
...
type ImportBusObservatoryGtfsRtResult = {
  runId: string;
  month: string;
...
function normalizeRouteId(row: Record<string, string>): {
  ...
  if (explicitRouteId !== null) {
    return { sourceRouteId, routeId: explicitRouteId };
  }

  if (sourceRouteId === null) {
    return { sourceRouteId, routeId: null };
  }
...
function parseCanonicalRow(headers: string[], line: string): CanonicalBusObservatoryRow {
  ...
  if (entityId === null) {
    throw new Error("Missing entity_id, vehicle_id, and trip_id");
  }
...
export async function importBusObservatoryGtfsRt(
  args: ImportBusObservatoryGtfsRtArgs,
): Promise<ImportBusObservatoryGtfsRtResult> {
  const runId = requireString("--run-id", args.runId);
  const year = requireNumber("--year", args.year);
  const month = requireNumber("--month", args.month);
  const csvPath = requireString("--canonical-csv", args.canonicalCsv);
  const sampleSeconds = args.sampleSeconds ?? 30;
  const maxGapSeconds = args.maxGapSeconds ?? 300;
  const options = createDbContext(args);
  const parsedAt = new Date().toISOString();
  const monthLabel = isoMonth(year, month);
  ...
  return withLocalPipelineDb(options.dbPath, async (local) => {
    ...
    for await (const line of readLines(csvPath)) {
      ...
      try {
        ...
        if (row.routeId !== null) {
          state.routeIds.add(row.routeId);
        }
        if (row.vehicleId !== null) {
          state.vehicleIds.add(row.vehicleId);
        }
      } catch {
    ...
    addQaIssue(
      issues,
      state.minTimestamp !== null &&
        state.minTimestamp > monthStartUnix(year, month) + maxGapSeconds,
...
function parseCliArgs(args: string[]): ImportBusObservatoryGtfsRtArgs {
  return parseCliOptions(args, {} as ImportBusObservatoryGtfsRtArgs, [
    {
      flags: ["--run-id"],
      apply: (target, value) => {
        if (value !== undefined) {
    ...
    yearOption<ImportBusObservatoryGtfsRtArgs>(),
    monthOption<ImportBusObservatoryGtfsRtArgs>(),
    {
      flags: ["--canonical-csv"],
      apply: (target, value) => {
        if (value !== undefined) {
          target.canonicalCsv = fromCliPath(value);
        }
      },
    },
    {
    ...
    {
      flags: ["--max-gap-seconds"],
      apply: (target, value) => {
        target.maxGapSeconds = Number(value);
      },
    },
    {
      flags: ["--db"],
    ...
    } satisfies CliOption<ImportBusObservatoryGtfsRtArgs>,
  ]);
}

export async function importBusObservatoryGtfsRtFromCli(
  args: string[],
): Promise<ImportBusObservatoryGtfsRtResult> {
  return importBusObservatoryGtfsRt(parseCliArgs(args));
}
