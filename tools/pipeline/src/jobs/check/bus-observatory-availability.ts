...
type BusObservatoryAvailabilityArgs = {
  year?: number;
  month?: number;
  artifactRoot?: string;
  output?: string;
...
function parseArgs(args: string[]): BusObservatoryAvailabilityArgs {
  const output: BusObservatoryAvailabilityArgs = {};
  const options: CliOption<BusObservatoryAvailabilityArgs>[] = [
    yearOption<BusObservatoryAvailabilityArgs>(),
    monthOption<BusObservatoryAvailabilityArgs>(),
    {
      flags: ["--artifact-root"],
      apply: (target, value) => {
        if (value !== undefined) {
...
export async function checkBusObservatoryAvailability(
  args: BusObservatoryAvailabilityArgs = {},
  ...
  const outputPath = args.output ?? defaultOutputPath(artifactRoot, requestedMonth);
...
export async function checkBusObservatoryAvailabilityFromCli(
  args: string[],
): Promise<BusObservatoryAvailabilityResult> {
  return checkBusObservatoryAvailability(parseArgs(args));
}
