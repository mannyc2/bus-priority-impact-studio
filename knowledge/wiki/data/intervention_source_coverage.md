...
| 4 | Date precision and event status | ACE/ABLE exact dates and bus-lane parseable `open_dates`; source gaps for missing dates; B46 ref-first timeline curation showed many source-visible dates remain unnormalized when expressed as seasons, bare years, year ranges, or `TBD` | A common runner-owned date model for exact date, month, season, year, range, planned date, warning-period start, summons start, completion, and unknown date; deterministic parsers first, with a narrow reviewed `rawDateText -> normalizedDateAssertion` codec only if deterministic coverage stalls | Every promoted intervention records `implementationDate`, `datePrecision`, `eventStatus`, and source-backed date assertion refs; unknown dates remain source gaps |
