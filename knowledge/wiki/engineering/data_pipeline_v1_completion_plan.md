...
- Monthly public-source refresh design: add a scheduled watcher for MTA Bus Route Segment Speeds and related monthly public datasets. It should detect the latest available `(year, month)`, distinguish schedule-only months from months with real speed rows, snapshot source metadata, and rerun `ingest:route-coverage`, network build/finalize, D1 export, and static artifact verification when a new complete month appears.
