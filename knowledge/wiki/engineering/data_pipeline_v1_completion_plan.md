...
- `bun run check` passed on 2026-05-17 after the production source-refresh work: typecheck, Biome style, architecture checks, package/pipeline/domain/source/db unit tests, web fixture tests, and Cloudflare Worker tests were green. This is a code/contract verification baseline, not a strict v1 data-release signal.
...
- `audit:pipeline-v1` writes a prompt-to-artifact completion audit that combines the public-source month, realtime month, structural/strict gates, GTFS-RT preflight, source coverage summaries, the route-speed source availability artifact, the source-refresh plan artifact, and a pass/partial/blocked checklist. The current March + May audit is intentionally blocked rather than green. If public/realtime months align but `data/artifacts/source-refresh/plan.json` is missing, the single-month source availability row is partial until the production collector/watcher handoff is recorded.
