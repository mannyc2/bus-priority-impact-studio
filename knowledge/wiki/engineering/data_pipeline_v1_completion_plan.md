...
- `bun run check:bus-observatory-gtfs-rt -- --year 2026 --month 3` inventories third-party Bus Observatory NYC bus GTFS-RT Parquet files under the public `busobservatory-lake` S3 bucket. On 2026-05-17 it found all 31 March-labeled files plus the 2026-04-01 bridge file, 32 files total, 3,591,483,083 bytes, and wrote `data/artifacts/source-availability/bus-observatory-gtfs-rt-2026-03.json` with `coverage.status = full_month_candidate` and `candidateLabel = third_party_full_month_candidate_pending_row_level_qa`. This is not official MTA backfill and is not yet an observed release; it requires row-level Parquet QA, route coverage QA, and an importer with `gtfsRtSource = third_party_recovered`.
- `bun run plan:source-refresh -- --start-year 2026 --end-year 2026 --year 2026 --month 5 --last-built-year 2026 --last-built-month 3 --min-speed-routes 300` writes `data/artifacts/source-refresh/plan.json`, preserving the GTFS-RT collector requirement and the monthly route-speed watcher decision in one handoff artifact. On 2026-05-17 it marked the GTFS-RT collector `required` and the monthly watcher `idle` because May speed data is still missing and March is already built.
...
- `bun --filter @bp/pipeline audit:pipeline-v1 -- --public-year 2026 --public-month 3 --realtime-year 2026 --realtime-month 5 --run-id gtfs-rt-v1-20260517T022348Z` writes `data/artifacts/pipeline-v1/audit-2026-03-2026-05.json`. The audit now treats March as the canonical public-source monthly release and May as a labeled realtime observed appendix. The strict same-month gate is preserved as `releaseModel.sameMonthPromotionReady`, not as the v1 release boundary.
- The audit also records `interventions.methodologyGate`, currently `descriptive_only` with `causalClaimsAllowed=false`, because external transit-domain methodology review is still open and bus-lane implementation-date source gaps remain.
...
- `bun run check` passed on 2026-05-17 after the production source-refresh work: typecheck, Biome style, architecture checks, package/pipeline/domain/source/db unit tests, web fixture tests, and Cloudflare Worker tests were green. This is a code/contract verification baseline; the v1 data-release signal is the audit artifact for the latest complete public-source month plus any labeled realtime appendix.
...
- An observed monthly promotion can be produced by collecting realtime during a month, waiting for that month's public speed rows, then running strict `finalize:pipeline-v1` and `audit:pipeline-v1`.
...
4. Observed GTFS-RT evidence is either aligned with the public-source month or attached as a clearly labeled current realtime appendix; structural fallback rows alone are not represented as observed reliability.
5. Observed headway, bunching, long-gap, and wait-time reliability metrics are computed from collected GTFS-RT data when a realtime appendix or promotion month is available.
