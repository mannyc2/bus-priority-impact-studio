...
- **Parking:** keep `release_context_only`. Raw backfill and geocode attempts are complete, but
  source location quality leaves a low route-touch rate, so parking should not support
  longitudinal detector claims without a separate source-quality breakthrough.
