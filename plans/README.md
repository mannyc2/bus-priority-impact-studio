...
| 121 | Calm network map: hover never dims, sr-only title band, honest chrome (no single-option toggles, borough selector deleted, structured note) | P1 | M-L | none | DONE (executed 2026-08-04; no drift from `e0c00aaf` in the four watched files). All eleven target behaviours landed. Hover/list-pointer now resolve to `preview` only; dim reserved for pin + keyboard focus, both of which now OUTRANK the pointer; `HOVER_DIM_DELAY_MS`, the latch and the `focusElsewhere` argument are gone; hysteresis and the 18px hit layer kept; hover width 5.5→4; 200 ms `line-opacity-transition` on the three network layers; ranked row memoised. Title band → `sr-only` h1 with coverage + verified count + Data notes relocated into `NetworkMapControls` (NOT the legend, which target behaviour 11 now allows to render nothing); error-branch h1 stays visible. Single-option toggles suppressed at `SingleToggle` (covers desktop + mobile); borough selector deleted from both mounts with `?borough=` parsing/filtering/fit preserved and pinned by a new test; map note renders lead/body/hint as three lines with one hint per encoding; all-zero legend returns null. Two deviations from the reference tree, both deliberate: its `LENS_COVERAGE_FLOOR` change was NOT ported (plan scopes lens/delay-coverage gating out — it remains unlanded), and its borough RESTYLE was superseded by the operator's delete direction. One simplification: `previewRouteId` resolves to `preview` rather than being renamed, since the keyboard path already flows through `focusedRouteId` — so the dim has two sources, not the plan's three. Decisions recorded in `knowledge/log.md` (supersedes Plan 080's hover-emphasis permission and the 059/080 title band). 449 web tests, `check:types`, `check:architecture`, `check:knowledge`, `@bp/web build` (137.7 KB gz entry; total JS 415.3→402.1 KB as the Select drops out of the map chunk) all green. No live browser check was run.) |
| 122 | Route-detail hygiene: one speed scalar, honest history states, map hover, treatment labels + popover, no filler card | P1 | M-L | 115 recommended first | DONE except step 6 (executed 2026-08-04; no drift from `e0c00aaf`). Steps 1-5 and 7-9 landed: Overview's speed prose deleted and the Slow-segments readout re-sourced to the header's scalar (labelled "route average"); `error` added to both artifact hook states so a failed fetch stops reading as a fact about the route; `segmentHistorySeries` returns a `reason` discriminant (`missing` / `spine_unclassified` / `needs_pattern_review` / `failed`) and the readout says which — the live case is `spine_unclassified`, where 36 served months were being discarded silently; route-map hover gained a same-id guard and an else-clear (a direction-filtered feature used to leave a stale hover standing) and feature-state writes dropped from N-per-transition to at most four; the inert `map` surface gate replaced with the unconditional form plus the eight real surface keys named; the unplanned "No flags raised" card deleted (its `capability` prop went with it); `other_documented` labels resolve through a curated map + a label≠kind guard so slugs never render, fixed at the crosswalk source too via one `reviewedOther` change; overflow popover renders the badge only when there IS a code, wraps long tokens, and its sr-only summary ends each entry with a period. **Step 6 BLOCKED**: the stale schemaVersion-1 `route-capability-manifest.json` IS read by path (`seed:local-capability-r2` in package.json), so it cannot be deleted, and regenerating needs `pipeline export d1 --db data/local/pipeline.sqlite`, which does not exist in this checkout — hand-editing the JSON is forbidden by the plan. Fixture left untouched; needs an operator run. Note for a later sweep: `route-detail.tsx`'s map-only fallback stays unreachable via the capability path. The reviewed-vocabulary hash was deliberately re-pinned (`663db5b7…`) with a new slug-shaped-label assertion beside it. 470 web tests, 51 analytics, 542 pipeline-v2, `check:types`, `check:architecture`, `@bp/web build` (137.9 KB gz entry, 402.6 KB total) all green.) |
| 123 | "Route at a glance" Overview enrichment spike (comp-gated) | P2 | M | 122, 126; operator comp token | TODO |
| 124 | Routes-index trend honesty + capability vocabulary out of public copy + search-field unification | P1 | S-M | none (data half rides 116's amended gates) | DONE (executed 2026-08-04; no drift from `e0c00aaf`). Null-spark trend cell renders an em dash instead of nothing (no fabricated flat line — the data half stays Plan 116's gate). Capability `reason` no longer reaches readers: dev-only in `HonestEmptySection`, removed from `rider-impact-summary` and — found by step 3's sweep, NOT in the plan's audit — `ReliabilitySection`. Dropping the last two consumers left `capability` as a dead parameter on `riderImpactSummary`, so it and its plumbing went too. `checked_clean` reworded per the amendment (badge "None", "Nothing on record", "We checked this release and found nothing to report for this route."); Building/Thin/Blocked verbatim; `coverage-matrix.ts` About-this-data labels untouched. /routes filter input back on the `SearchField` primitive with no border/background/shadow overrides. Recorded in `knowledge/log.md`. New `honest-empty-section.test.ts` plus trend-cell cases; 459 web tests, `check:types`, `check:architecture`, `check:knowledge`, `@bp/web build` all green. NOT done: the plan's step-5 side-by-side screenshot against the homepage input — no browser was run; the override removal is verified by source only.) |
| 125 | One click surface on the network map (popup leads; rail stays on browse) | P2 | S-M | 121 (same file, sequential) | DONE (executed 2026-08-04 after 121; drift was exactly 121's edits). Desktop click now opens ONE surface: the anchored popup. The rail no longer auto-swaps into a card restating the same metrics; it opens for a shared `?segment=` link only, so that evidence list and its canonicalization notice keep their render surface, and the mobile sheet is untouched. Per-route evidence is fetched only where something reads it — a plain desktop pin no longer fires the request. The popup gains the one line the retired card uniquely carried: `#N of M in this view`. Three decisions were extracted as exported pure helpers (`networkMapRailSurface`, `selectedEvidenceWanted`, `popupRankLine`) following the file's existing `selectedRouteEvidenceKey` pattern, since no page-render harness exists — that is what the new tests drive. 454 web tests, `check:types`, `check:architecture`, `@bp/web build` (137.9 KB gz entry, 402.3 KB total) all green.) |
| 126 | One route map: Overview map goes interactive (popup + direction); explorer map + readout rail retire; riders chart on the chart kit; charts fill cards | P1 | L | 122 (same files); before 123 | DONE (executed 2026-08-04 on top of 122; drift was exactly 122's edits). The route page mounts exactly one `RouteMapLibre`, in Overview's Route-map card via the new `RouteSegmentMapCard` (geo + lanes fetch + direction chips + overlay toggle + legend + popup). One click surface: an anchored popup naming the segment, its direction, its observed all-day speed, its rank in the active ranking, and its lane line; ✕ / Esc / background click close it and restore focus to the map frame. The explorer's map column, `SegmentReadout`, `SegmentSpeedLegend`, `useBusLanes` and `mapOnly` are deleted (−370 lines there); the ranked list is full width and a shared `?segment=` scrolls its row into view. `?segment=`/`?direction=`/`?lanes=` now validate on the Overview tab too (no new param names); `month`/`daypart` stay segments-only, since only the list renders them. "When riders ride" moved onto the lazy Recharts pair with the app tooltip and a ReferenceLine peak label. `ChartFrame` gained an additive `fill` and `SpeedTrend` a string height, so Overview's speed chart matches the map card instead of leaving dead space under a 172px body. **Five deviations, all deliberate:** (1) the direction chips were ADDED to the map card rather than MOVED — removing them from the list would leave the ranked list with no way to unfilter itself, and they write the same shared param either way; (2) the `symbol-placement: line` arrow layer was skipped under the plan's own escape clause — `mapBaseStyle()` declares no `glyphs`, so a text arrow would mean adding a font endpoint; the popup and chips carry direction instead; (3) the popup keeps the readout's lane line and its per-segment "What changed" link, so the gen-18 D2 route survives at segment grain and `laneReadoutLine` is not orphaned; (4) "Explore route segments" switches tabs through the search state rather than the section callback, so the direction and overlay just set survive the trip; (5) the previously-dead `compact` prop was repurposed as the 380/320 card size, and the no-WebGL SVG fallback follows it to `variant="mini"`. **One behaviour change worth naming:** retiring the `map` section removed an always-render member from the Slow-segments tab, so the tab now reports its ranked list's real readiness (`checked_clean`/`building`) instead of borrowing the map's unconditional render — the honest outcome, pinned by updated registry tests. Recharts and MapLibre both render nothing under a server render, so popup content, segment anchoring and the riders bars are tested as pure models (the `buildSpeedTrendChartModel` pattern). 488 web tests, `check:types`, `check:architecture`, `@bp/web build` (137.6 KB gz entry, 402.1 KB total — maplibre still a split chunk) all green. No live browser check was run.) |
