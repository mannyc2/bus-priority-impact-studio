...
Before any new run, the external exact-route task
`019f7640-fd5c-7be2-8a40-a7c264284c0f` must merge its manifest-v5 Tracker
consumer and green B44/B44+ identity/evidence fixtures. Import the next
strictly verified, non-quarantined named Wiki release through that path,
produce a wholly new candidate set and hashes, close the control-
contamination and bounded-scope defects, and obtain a fresh complete receipt
for that exact set. No rc19/rc22/rc23 recommendation, decision, or approval is
carried forward. The immutable historical five-study output remains only a
historical artifact and is not regenerated by this amendment.
