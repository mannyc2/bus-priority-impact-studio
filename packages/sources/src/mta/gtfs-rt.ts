...
export function normalizeGtfsRtRouteId(value: unknown): string | null {
...
function commonTripFields(trip: PlainTripDescriptor | undefined) {
  ...
  return {
    sourceRouteId,
    routeId: normalizeGtfsRtRouteId(sourceRouteId),
...
function decodeFeedObject(bytes: Uint8Array): PlainFeedMessage {
...
export function parseGtfsRtFeed(bytes: Uint8Array): NormalizedGtfsRtFeed {
  const feed = decodeFeedObject(bytes);
