...
function extractResult(body: Record<string, unknown>, key: string): GeoclientResult | null {
  const detail = (body[key] ?? body) as Record<string, unknown> | undefined;
  if (!detail) return null;
  const physicalId =
    typeof detail["physicalId"] === "string"
      ? (detail["physicalId"] as string)
      : typeof detail["workAreaFormatIdLeft"] === "string"
        ? (detail["workAreaFormatIdLeft"] as string)
        : null;
  ...
  const lat = typeof detail["latitude"] === "number" ? (detail["latitude"] as number) : null;
  const lng = typeof detail["longitude"] === "number" ? (detail["longitude"] as number) : null;
  const confidence =
    typeof detail["geosupportReturnCode"] === "string"
      ? `grc=${detail["geosupportReturnCode"] as string}`
      : "ok";
