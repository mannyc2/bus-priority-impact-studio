...
// Reviewed-gold labels for the `service_request_context` detector (ADR-0018 steps 3-5).
// Wave 4 family adaptation (associational context). Standard 5-bucket frontend-use vocabulary applies,
// but `primary_finding` is rare by design — 311 complaints are broad street-condition context with
// reporting bias, not causal evidence — so the expected label mass is `route_context`/`suppress` and
// the eval lens is leakage INTO findings (`findingsLeakageCount`), not primary survival. Calibration
// tags emphasize route fanout, low match weight, thin high-confidence touches, complaint-type
// relevance, reporting bias, and the "not a causal attribution" caveat.

export type ServiceRequestContextReviewedFrontendUse =
  | "primary_finding"
  | "route_context"
  | "reviewer_only"
  | "needs_more_evidence"
...
export type ServiceRequestContextCalibrationTag =
  | "true_service_request_context"
  | "slow_route_supported"
  | "high_confidence_311_touches"
  | "high_route_fanout"
  | "low_match_weight"
  | "thin_high_confidence_touches"
  | "complaint_type_not_bus_relevant"
  | "reporting_bias"
  | "broad_street_condition_not_specific"
  | "near_threshold"
  | "not_a_causal_attribution"
  | "not_actionable_as_claim";

export type ServiceRequestContextReviewedDecision = {
  readonly detectorId: string;
  readonly scopeId: string;
  readonly routeId: string | null;
  readonly sourceQueue: "candidate_review" | "skipped_control" | "clean_no_hit_control";
  readonly expectedFrontendUse: ServiceRequestContextReviewedFrontendUse;
  readonly calibrationTags: readonly ServiceRequestContextCalibrationTag[];
...
export type ServiceRequestContextGoldLabel = ServiceRequestContextReviewedDecision & {
...
export type ServiceRequestContextReviewedGoldArtifact = {
  readonly artifactKind: "service_request_context_reviewed_gold";
  ...
  readonly labels: readonly ServiceRequestContextGoldLabel[];
};

export type ServiceRequestContextCandidateLike = {
...
export type ServiceRequestContextCoverageLike = {
  readonly detectorId?: unknown;
  readonly routeId?: unknown;
  readonly scopeId?: unknown;
...
export type ServiceRequestContextReviewedGoldEvaluation = {
  readonly artifactKind: "service_request_context_reviewed_gold_evaluation";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly byExpectedFrontendUse: Record<
    ServiceRequestContextReviewedFrontendUse,
  ...
  readonly byReviewDepth: Record<
    ...
    { readonly expected: number; readonly emitted: number; readonly dropped: number }
  >;
  readonly emittedReviewed: readonly ServiceRequestContextGoldLabel[];
  readonly droppedReviewed: readonly ServiceRequestContextGoldLabel[];
  readonly unreviewedEmittedCandidates: readonly ServiceRequestContextCandidateLike[];
};

export type ServiceRequestContextReadinessProjection = {
  readonly artifactKind: "service_request_context_readiness_projection";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly items: readonly {
    ...
    readonly reviewedFrontendUse: ServiceRequestContextReviewedFrontendUse | "unreviewed";
    readonly emittedCandidate: boolean;
    readonly reasonCode: string | null;
    readonly calibrationTags: readonly ServiceRequestContextCalibrationTag[];
    readonly label: ServiceRequestContextGoldLabel | null;
    readonly candidate: ServiceRequestContextCandidateLike | null;
  }[];
};

export type BuildServiceRequestContextReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly reviewQueuePath: string;
  readonly decisionsPath: string;
  readonly decisions: readonly ServiceRequestContextReviewedDecision[];
};

export type EvaluateServiceRequestContextReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly gold: ServiceRequestContextReviewedGoldArtifact;
  readonly candidates: readonly ServiceRequestContextCandidateLike[];
};

export type BuildServiceRequestContextReadinessProjectionInput =
  EvaluateServiceRequestContextReviewedGoldInput & {
    readonly coverage: readonly ServiceRequestContextCoverageLike[];
  };
...
export function buildServiceRequestContextReviewedGoldArtifact(
  input: BuildServiceRequestContextReviewedGoldInput,
): ServiceRequestContextReviewedGoldArtifact {
  const byCalibrationTag = new Map<ServiceRequestContextCalibrationTag, number>();
  const byReviewDepth = new Map<string, number>();
  const labels = input.decisions
    .map((decision): ServiceRequestContextGoldLabel => {
  ...
  return {
    artifactKind: "service_request_context_reviewed_gold",
    schemaVersion: 1,
...
export function evaluateServiceRequestContextReviewedGold(
  input: EvaluateServiceRequestContextReviewedGoldInput,
): ServiceRequestContextReviewedGoldEvaluation {
  ...
  const byCalibrationTag = new Map<
    ServiceRequestContextCalibrationTag,
    { expected: number; emitted: number; dropped: number }
  >();
  ...
  return {
    artifactKind: "service_request_context_reviewed_gold_evaluation",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
...
export function buildServiceRequestContextReadinessProjection(
  input: BuildServiceRequestContextReadinessProjectionInput,
): ServiceRequestContextReadinessProjection {
  ...
  return {
    artifactKind: "service_request_context_readiness_projection",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
