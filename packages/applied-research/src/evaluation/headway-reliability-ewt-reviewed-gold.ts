...
// Reviewed-gold, evaluation, and readiness machinery for the `headway_reliability_ewt` detector
// (ADR-0018 steps 3-6). The detector scopes claims to stop-direction-hour cells, so labels carry
// stop-direction-hour scope identity and the calibration vocabulary captures the cell-level risks:
// thin headway samples, borderline frequent service, extreme variability that may be a feed artifact,
// and the standing caveat that a single stop-hour cell should not be generalized to a whole route.

export type HeadwayReliabilityEwtReviewedFrontendUse =
  | "primary_finding"
  | "route_context"
  | "reviewer_only"
  | "needs_more_evidence"
...
export type HeadwayReliabilityEwtCalibrationTag =
  | "true_excess_wait_shortfall"
  | "stop_direction_hour_cell"
  | "frequent_service_supported"
  | "high_headway_samples"
  | "extreme_variability_los_f"
  | "single_cell_not_route_generalizable"
  | "thin_headway_samples"
  | "borderline_frequency"
  | "near_threshold"
  | "terminal_or_layover_artifact"
  | "feed_gap_or_coverage_artifact"
  | "not_actionable_as_claim";

export type HeadwayReliabilityEwtReviewedDecision = {
  readonly detectorId: string;
  readonly scopeId: string;
  readonly routeId: string | null;
  readonly sourceQueue: "candidate_review" | "skipped_control" | "clean_no_hit_control";
  readonly expectedFrontendUse: HeadwayReliabilityEwtReviewedFrontendUse;
  readonly calibrationTags: readonly HeadwayReliabilityEwtCalibrationTag[];
...
export type HeadwayReliabilityEwtGoldLabel = HeadwayReliabilityEwtReviewedDecision & {
...
export type HeadwayReliabilityEwtReviewedGoldArtifact = {
  readonly artifactKind: "headway_reliability_ewt_reviewed_gold";
  ...
  readonly labels: readonly HeadwayReliabilityEwtGoldLabel[];
};

export type HeadwayReliabilityEwtCandidateLike = {
...
export type HeadwayReliabilityEwtCoverageLike = {
  readonly detectorId?: unknown;
  readonly routeId?: unknown;
  readonly scopeId?: unknown;
...
export type HeadwayReliabilityEwtReviewedGoldEvaluation = {
  readonly artifactKind: "headway_reliability_ewt_reviewed_gold_evaluation";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly byExpectedFrontendUse: Record<
    HeadwayReliabilityEwtReviewedFrontendUse,
  ...
  readonly byReviewDepth: Record<
    ...
    { readonly expected: number; readonly emitted: number; readonly dropped: number }
  >;
  readonly emittedReviewed: readonly HeadwayReliabilityEwtGoldLabel[];
  readonly droppedReviewed: readonly HeadwayReliabilityEwtGoldLabel[];
  readonly unreviewedEmittedCandidates: readonly HeadwayReliabilityEwtCandidateLike[];
};

export type HeadwayReliabilityEwtReadinessProjection = {
  readonly artifactKind: "headway_reliability_ewt_readiness_projection";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly items: readonly {
    ...
    readonly reviewedFrontendUse: HeadwayReliabilityEwtReviewedFrontendUse | "unreviewed";
    readonly emittedCandidate: boolean;
    readonly reasonCode: string | null;
    readonly calibrationTags: readonly HeadwayReliabilityEwtCalibrationTag[];
    readonly label: HeadwayReliabilityEwtGoldLabel | null;
    readonly candidate: HeadwayReliabilityEwtCandidateLike | null;
  }[];
};

export type BuildHeadwayReliabilityEwtReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly reviewQueuePath: string;
  readonly decisionsPath: string;
  readonly decisions: readonly HeadwayReliabilityEwtReviewedDecision[];
};

export type EvaluateHeadwayReliabilityEwtReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly gold: HeadwayReliabilityEwtReviewedGoldArtifact;
  readonly candidates: readonly HeadwayReliabilityEwtCandidateLike[];
};

export type BuildHeadwayReliabilityEwtReadinessProjectionInput =
  EvaluateHeadwayReliabilityEwtReviewedGoldInput & {
    readonly coverage: readonly HeadwayReliabilityEwtCoverageLike[];
  };
...
export function buildHeadwayReliabilityEwtReviewedGoldArtifact(
  input: BuildHeadwayReliabilityEwtReviewedGoldInput,
): HeadwayReliabilityEwtReviewedGoldArtifact {
  const byCalibrationTag = new Map<HeadwayReliabilityEwtCalibrationTag, number>();
  const byReviewDepth = new Map<string, number>();
  const labels = input.decisions
    .map((decision): HeadwayReliabilityEwtGoldLabel => {
  ...
  return {
    artifactKind: "headway_reliability_ewt_reviewed_gold",
    schemaVersion: 1,
...
export function evaluateHeadwayReliabilityEwtReviewedGold(
  input: EvaluateHeadwayReliabilityEwtReviewedGoldInput,
): HeadwayReliabilityEwtReviewedGoldEvaluation {
  ...
  const byCalibrationTag = new Map<
    HeadwayReliabilityEwtCalibrationTag,
    { expected: number; emitted: number; dropped: number }
  >();
  ...
  return {
    artifactKind: "headway_reliability_ewt_reviewed_gold_evaluation",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
...
export function buildHeadwayReliabilityEwtReadinessProjection(
  input: BuildHeadwayReliabilityEwtReadinessProjectionInput,
): HeadwayReliabilityEwtReadinessProjection {
  ...
  return {
    artifactKind: "headway_reliability_ewt_readiness_projection",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
