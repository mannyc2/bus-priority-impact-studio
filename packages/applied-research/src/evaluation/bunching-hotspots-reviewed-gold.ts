...
// Reviewed-gold, evaluation, and readiness machinery for the `bunching_hotspots` detector
// (ADR-0018 steps 3-6). The detector scopes claims to stop-direction-hour cells and emits two reason
// classes (bunching vs headway gap), so the calibration vocabulary captures both, plus the cell-level
// risks: thin headway-pair support, extreme shares that may be GPS arrival noise, terminal dispatch
// artifacts, and the standing caveat that a single stop-hour cell should not be generalized to a
// whole route.

export type BunchingHotspotsReviewedFrontendUse =
  | "primary_finding"
  | "route_context"
  | "reviewer_only"
  | "needs_more_evidence"
...
export type BunchingHotspotsCalibrationTag =
  | "true_bunching_hotspot"
  | "true_headway_gap_hotspot"
  | "stop_direction_hour_cell"
  | "high_pair_support"
  | "gps_arrival_noise_artifact"
  | "terminal_dispatch_artifact"
...
export type BunchingHotspotsReviewedDecision = {
  readonly detectorId: string;
  readonly scopeId: string;
  readonly routeId: string | null;
  readonly sourceQueue: "candidate_review" | "skipped_control" | "clean_no_hit_control";
  readonly expectedFrontendUse: BunchingHotspotsReviewedFrontendUse;
  readonly calibrationTags: readonly BunchingHotspotsCalibrationTag[];
...
export type BunchingHotspotsGoldLabel = BunchingHotspotsReviewedDecision & {
...
export type BunchingHotspotsReviewedGoldArtifact = {
  readonly artifactKind: "bunching_hotspots_reviewed_gold";
  ...
  readonly labels: readonly BunchingHotspotsGoldLabel[];
};

export type BunchingHotspotsCandidateLike = {
...
export type BunchingHotspotsCoverageLike = {
  readonly detectorId?: unknown;
  readonly routeId?: unknown;
  readonly scopeId?: unknown;
...
export type BunchingHotspotsReviewedGoldEvaluation = {
  readonly artifactKind: "bunching_hotspots_reviewed_gold_evaluation";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly byExpectedFrontendUse: Record<
    BunchingHotspotsReviewedFrontendUse,
  ...
  readonly byReviewDepth: Record<
    ...
    { readonly expected: number; readonly emitted: number; readonly dropped: number }
  >;
  readonly emittedReviewed: readonly BunchingHotspotsGoldLabel[];
  readonly droppedReviewed: readonly BunchingHotspotsGoldLabel[];
  readonly unreviewedEmittedCandidates: readonly BunchingHotspotsCandidateLike[];
};

export type BunchingHotspotsReadinessProjection = {
  readonly artifactKind: "bunching_hotspots_readiness_projection";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly items: readonly {
    ...
    readonly reviewedFrontendUse: BunchingHotspotsReviewedFrontendUse | "unreviewed";
    readonly emittedCandidate: boolean;
    readonly reasonCode: string | null;
    readonly calibrationTags: readonly BunchingHotspotsCalibrationTag[];
    readonly label: BunchingHotspotsGoldLabel | null;
    readonly candidate: BunchingHotspotsCandidateLike | null;
  }[];
};

export type BuildBunchingHotspotsReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly reviewQueuePath: string;
  readonly decisionsPath: string;
  readonly decisions: readonly BunchingHotspotsReviewedDecision[];
};

export type EvaluateBunchingHotspotsReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly gold: BunchingHotspotsReviewedGoldArtifact;
  readonly candidates: readonly BunchingHotspotsCandidateLike[];
};

export type BuildBunchingHotspotsReadinessProjectionInput =
  EvaluateBunchingHotspotsReviewedGoldInput & {
    readonly coverage: readonly BunchingHotspotsCoverageLike[];
  };
...
export function buildBunchingHotspotsReviewedGoldArtifact(
  input: BuildBunchingHotspotsReviewedGoldInput,
): BunchingHotspotsReviewedGoldArtifact {
  const byCalibrationTag = new Map<BunchingHotspotsCalibrationTag, number>();
  const byReviewDepth = new Map<string, number>();
  const labels = input.decisions
    .map((decision): BunchingHotspotsGoldLabel => {
  ...
  return {
    artifactKind: "bunching_hotspots_reviewed_gold",
    schemaVersion: 1,
...
export function evaluateBunchingHotspotsReviewedGold(
  input: EvaluateBunchingHotspotsReviewedGoldInput,
): BunchingHotspotsReviewedGoldEvaluation {
  ...
  const byCalibrationTag = new Map<
    BunchingHotspotsCalibrationTag,
    { expected: number; emitted: number; dropped: number }
  >();
  ...
  return {
    artifactKind: "bunching_hotspots_reviewed_gold_evaluation",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
...
export function buildBunchingHotspotsReadinessProjection(
  input: BuildBunchingHotspotsReadinessProjectionInput,
): BunchingHotspotsReadinessProjection {
  ...
  return {
    artifactKind: "bunching_hotspots_readiness_projection",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
