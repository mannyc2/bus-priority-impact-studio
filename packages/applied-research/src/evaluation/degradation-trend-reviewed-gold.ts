...
// Reviewed-gold labels for the `degradation_trend` detector (ADR-0018 steps 3-5).
// Standard public-facing trend detector (Wave 2 #5); the shared 5-bucket frontend-use vocabulary
// applies. Calibration tags emphasize the history-confidence failure modes (thin history, short prior
// baseline, route-version breaks, seasonality) the plan calls out. The eval reports suppress leakage +
// reviewed-primary survival; the projection buckets scopes into the shared readiness vocabulary.

export type DegradationTrendReviewedFrontendUse =
  | "primary_finding"
  | "route_context"
  | "reviewer_only"
  | "needs_more_evidence"
...
export type DegradationTrendCalibrationTag =
  | "true_worsening_trend"
  | "route_scope_trend"
  | "segment_scope_trend"
  | "history_supported"
  | "strong_robust_z"
  ...
  | "short_baseline"
  | "seasonal_artifact"
  | "route_version_break_confound"
  | "single_metric_not_corroborated"
  | "near_threshold"
  | "not_actionable_as_claim";

export type DegradationTrendReviewedDecision = {
  readonly detectorId: string;
  readonly scopeId: string;
  readonly routeId: string | null;
  readonly sourceQueue: "candidate_review" | "skipped_control" | "clean_no_hit_control";
  readonly expectedFrontendUse: DegradationTrendReviewedFrontendUse;
  readonly calibrationTags: readonly DegradationTrendCalibrationTag[];
...
export type DegradationTrendGoldLabel = DegradationTrendReviewedDecision & {
...
export type DegradationTrendReviewedGoldArtifact = {
  readonly artifactKind: "degradation_trend_reviewed_gold";
  ...
  readonly labels: readonly DegradationTrendGoldLabel[];
};

export type DegradationTrendCandidateLike = {
...
export type DegradationTrendCoverageLike = {
  readonly detectorId?: unknown;
  readonly routeId?: unknown;
  readonly scopeId?: unknown;
...
export type DegradationTrendReviewedGoldEvaluation = {
  readonly artifactKind: "degradation_trend_reviewed_gold_evaluation";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly byExpectedFrontendUse: Record<
    DegradationTrendReviewedFrontendUse,
  ...
  readonly byReviewDepth: Record<
    ...
    { readonly expected: number; readonly emitted: number; readonly dropped: number }
  >;
  readonly emittedReviewed: readonly DegradationTrendGoldLabel[];
  readonly droppedReviewed: readonly DegradationTrendGoldLabel[];
  readonly unreviewedEmittedCandidates: readonly DegradationTrendCandidateLike[];
};

export type DegradationTrendReadinessProjection = {
  readonly artifactKind: "degradation_trend_readiness_projection";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly items: readonly {
    ...
    readonly reviewedFrontendUse: DegradationTrendReviewedFrontendUse | "unreviewed";
    readonly emittedCandidate: boolean;
    readonly reasonCode: string | null;
    readonly calibrationTags: readonly DegradationTrendCalibrationTag[];
    readonly label: DegradationTrendGoldLabel | null;
    readonly candidate: DegradationTrendCandidateLike | null;
  }[];
};

export type BuildDegradationTrendReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly reviewQueuePath: string;
  readonly decisionsPath: string;
  readonly decisions: readonly DegradationTrendReviewedDecision[];
};

export type EvaluateDegradationTrendReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly gold: DegradationTrendReviewedGoldArtifact;
  readonly candidates: readonly DegradationTrendCandidateLike[];
};

export type BuildDegradationTrendReadinessProjectionInput =
  EvaluateDegradationTrendReviewedGoldInput & {
    readonly coverage: readonly DegradationTrendCoverageLike[];
  };
...
export function buildDegradationTrendReviewedGoldArtifact(
  input: BuildDegradationTrendReviewedGoldInput,
): DegradationTrendReviewedGoldArtifact {
  const byCalibrationTag = new Map<DegradationTrendCalibrationTag, number>();
  const byReviewDepth = new Map<string, number>();
  const labels = input.decisions
    .map((decision): DegradationTrendGoldLabel => {
  ...
  return {
    artifactKind: "degradation_trend_reviewed_gold",
    schemaVersion: 1,
...
export function evaluateDegradationTrendReviewedGold(
  input: EvaluateDegradationTrendReviewedGoldInput,
): DegradationTrendReviewedGoldEvaluation {
  ...
  const byCalibrationTag = new Map<
    DegradationTrendCalibrationTag,
    { expected: number; emitted: number; dropped: number }
  >();
  ...
  return {
    artifactKind: "degradation_trend_reviewed_gold_evaluation",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
...
export function buildDegradationTrendReadinessProjection(
  input: BuildDegradationTrendReadinessProjectionInput,
): DegradationTrendReadinessProjection {
  ...
  return {
    artifactKind: "degradation_trend_readiness_projection",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
