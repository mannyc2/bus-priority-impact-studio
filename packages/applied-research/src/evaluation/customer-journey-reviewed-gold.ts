...
export type CustomerJourneyReviewedFrontendUse =
  | "primary_finding"
  | "route_context"
  | "reviewer_only"
  | "needs_more_evidence"
...
export type CustomerJourneyRootCauseTag =
  | "true_customer_impact"
  | "composite_metric_ambiguous"
  | "wait_component_driven"
  | "in_vehicle_component_driven"
  | "service_delivery_component_driven"
  | "low_exposure"
  | "insufficient_history"
  | "missing_denominator"
  | "route_rollup_artifact"
  | "express_route_context"
  | "sbs_route_context"
  | "seasonality_or_calendar_artifact"
  | "not_actionable_as_claim";

export type CustomerJourneyReviewedDecision = {
  readonly detectorId: string;
  readonly scopeId: string;
  readonly routeId: string | null;
  readonly sourceQueue: "candidate_review" | "skipped_control" | "clean_no_hit_control";
  readonly expectedFrontendUse: CustomerJourneyReviewedFrontendUse;
  readonly rootCauseTags: readonly CustomerJourneyRootCauseTag[];
...
export type CustomerJourneyGoldLabel = CustomerJourneyReviewedDecision & {
...
export type CustomerJourneyReviewedGoldArtifact = {
  readonly artifactKind: "customer_journey_reviewed_gold";
  ...
  readonly summary: {
    ...
    readonly byRootCauseTag: Record<string, number>;
    readonly byReviewDepth: Record<string, number>;
  };
  readonly labels: readonly CustomerJourneyGoldLabel[];
};

export type CustomerJourneyCandidateLike = {
...
export type CustomerJourneyCoverageLike = {
  readonly detectorId?: unknown;
  readonly routeId?: unknown;
  readonly scopeId?: unknown;
...
export type CustomerJourneyReviewedGoldEvaluation = {
  readonly artifactKind: "customer_journey_reviewed_gold_evaluation";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly byReviewDepth: Record<
    ...
    { readonly expected: number; readonly emitted: number; readonly dropped: number }
  >;
  readonly emittedReviewed: readonly CustomerJourneyGoldLabel[];
  readonly droppedReviewed: readonly CustomerJourneyGoldLabel[];
  readonly unreviewedEmittedCandidates: readonly CustomerJourneyCandidateLike[];
};

export type CustomerJourneyReadinessProjection = {
  readonly artifactKind: "customer_journey_readiness_projection";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly items: readonly {
    ...
    readonly reviewedFrontendUse: CustomerJourneyReviewedFrontendUse | "unreviewed";
    readonly emittedCandidate: boolean;
    readonly reasonCode: string | null;
    readonly rootCauseTags: readonly CustomerJourneyRootCauseTag[];
    readonly label: CustomerJourneyGoldLabel | null;
    readonly candidate: CustomerJourneyCandidateLike | null;
  }[];
};

export type BuildCustomerJourneyReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly reviewQueuePath: string;
  readonly decisionsPath: string;
  readonly decisions: readonly CustomerJourneyReviewedDecision[];
};

export type EvaluateCustomerJourneyReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly gold: CustomerJourneyReviewedGoldArtifact;
  readonly candidates: readonly CustomerJourneyCandidateLike[];
};

export type BuildCustomerJourneyReadinessProjectionInput =
  EvaluateCustomerJourneyReviewedGoldInput & {
    readonly coverage: readonly CustomerJourneyCoverageLike[];
  };
...
export function buildCustomerJourneyReviewedGoldArtifact(
  input: BuildCustomerJourneyReviewedGoldInput,
): CustomerJourneyReviewedGoldArtifact {
  const byRootCauseTag = new Map<CustomerJourneyRootCauseTag, number>();
  const byReviewDepth = new Map<string, number>();
  const labels = input.decisions
    .map((decision): CustomerJourneyGoldLabel => {
      const identityKey = detectorScopeIdentityKey({
        detectorId: decision.detectorId,
        scopeId: decision.scopeId,
      });
      for (const tag of decision.rootCauseTags) increment(byRootCauseTag, tag);
  ...
  return {
    artifactKind: "customer_journey_reviewed_gold",
    schemaVersion: 1,
    ...
    summary: {
      ...
      byRootCauseTag: countRecord(byRootCauseTag),
...
export function evaluateCustomerJourneyReviewedGold(
  input: EvaluateCustomerJourneyReviewedGoldInput,
): CustomerJourneyReviewedGoldEvaluation {
  ...
  const byRootCauseTag = new Map<
    CustomerJourneyRootCauseTag,
    { expected: number; emitted: number; dropped: number }
  >();
  ...
  for (const label of input.gold.labels) {
    ...
    for (const tag of label.rootCauseTags) updateGroup(byRootCauseTag, tag, emitted);
  ...
  return {
    artifactKind: "customer_journey_reviewed_gold_evaluation",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
    ...
    byRootCauseTag: Object.fromEntries(
      [...byRootCauseTag.entries()].sort(([left], [right]) => left.localeCompare(right)),
    ),
...
export function buildCustomerJourneyReadinessProjection(
  input: BuildCustomerJourneyReadinessProjectionInput,
): CustomerJourneyReadinessProjection {
    ...
    return [
      {
        ...
        rootCauseTags: label?.rootCauseTags ?? [],
        label,
        candidate,
      },
    ];
