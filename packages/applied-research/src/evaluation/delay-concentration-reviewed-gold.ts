...
// Reviewed-gold labels for the `delay_concentration` detector (ADR-0018 Wave 1 #4).
// The detector is descriptive: public labels may say that avoidable delay is concentrated across a
// route's segments, but must not promote Gini/top-segment evidence as a causal diagnosis or treatment
// recommendation. Calibration tags emphasize segment-count and segment-mix sensitivity.

export type DelayConcentrationReviewedFrontendUse =
  | "primary_finding"
  | "route_context"
  | "reviewer_only"
  | "needs_more_evidence"
...
export type DelayConcentrationCalibrationTag =
  | "true_delay_concentration"
  | "route_scope_concentration"
  | "fleet_benchmark_supported"
  | "segment_support_supported"
  | "top_segments_plausible"
  | "low_eligible_segment_count"
  | "segment_count_sensitive"
  | "segment_length_mix_uncertain"
  | "single_segment_dominant"
  | "duplicate_or_stale_segment_rows"
  | "near_threshold"
  | "low_total_delay"
  | "not_causal_claim"
  | "not_actionable_as_claim";

export type DelayConcentrationReviewedDecision = {
  readonly detectorId: string;
  readonly scopeId: string;
  readonly routeId: string | null;
  readonly sourceQueue: "candidate_review" | "skipped_control" | "clean_no_hit_control";
  readonly expectedFrontendUse: DelayConcentrationReviewedFrontendUse;
  readonly calibrationTags: readonly DelayConcentrationCalibrationTag[];
...
export type DelayConcentrationGoldLabel = DelayConcentrationReviewedDecision & {
...
export type DelayConcentrationReviewedGoldArtifact = {
  readonly artifactKind: "delay_concentration_reviewed_gold";
  ...
  readonly labels: readonly DelayConcentrationGoldLabel[];
};

export type DelayConcentrationCandidateLike = {
...
export type DelayConcentrationCoverageLike = {
  readonly detectorId?: unknown;
  readonly routeId?: unknown;
  readonly scopeId?: unknown;
...
export type DelayConcentrationReviewedGoldEvaluation = {
  readonly artifactKind: "delay_concentration_reviewed_gold_evaluation";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly byExpectedFrontendUse: Record<
    DelayConcentrationReviewedFrontendUse,
  ...
  readonly byReviewDepth: Record<
    ...
    { readonly expected: number; readonly emitted: number; readonly dropped: number }
  >;
  readonly emittedReviewed: readonly DelayConcentrationGoldLabel[];
  readonly droppedReviewed: readonly DelayConcentrationGoldLabel[];
  readonly unreviewedEmittedCandidates: readonly DelayConcentrationCandidateLike[];
};

export type DelayConcentrationReadinessProjection = {
  readonly artifactKind: "delay_concentration_readiness_projection";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly items: readonly {
    ...
    readonly reviewedFrontendUse: DelayConcentrationReviewedFrontendUse | "unreviewed";
    readonly emittedCandidate: boolean;
    readonly reasonCode: string | null;
    readonly calibrationTags: readonly DelayConcentrationCalibrationTag[];
    readonly label: DelayConcentrationGoldLabel | null;
    readonly candidate: DelayConcentrationCandidateLike | null;
  }[];
};

export type BuildDelayConcentrationReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly reviewQueuePath: string;
  readonly decisionsPath: string;
  readonly decisions: readonly DelayConcentrationReviewedDecision[];
};

export type EvaluateDelayConcentrationReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly gold: DelayConcentrationReviewedGoldArtifact;
  readonly candidates: readonly DelayConcentrationCandidateLike[];
};

export type BuildDelayConcentrationReadinessProjectionInput =
  EvaluateDelayConcentrationReviewedGoldInput & {
    readonly coverage: readonly DelayConcentrationCoverageLike[];
  };
...
export function buildDelayConcentrationReviewedGoldArtifact(
  input: BuildDelayConcentrationReviewedGoldInput,
): DelayConcentrationReviewedGoldArtifact {
  const byCalibrationTag = new Map<DelayConcentrationCalibrationTag, number>();
  const byReviewDepth = new Map<string, number>();
  const labels = input.decisions
    .map((decision): DelayConcentrationGoldLabel => {
  ...
  return {
    artifactKind: "delay_concentration_reviewed_gold",
    schemaVersion: 1,
...
export function evaluateDelayConcentrationReviewedGold(
  input: EvaluateDelayConcentrationReviewedGoldInput,
): DelayConcentrationReviewedGoldEvaluation {
  ...
  const byCalibrationTag = new Map<
    DelayConcentrationCalibrationTag,
    { expected: number; emitted: number; dropped: number }
  >();
  ...
  return {
    artifactKind: "delay_concentration_reviewed_gold_evaluation",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
...
export function buildDelayConcentrationReadinessProjection(
  input: BuildDelayConcentrationReadinessProjectionInput,
): DelayConcentrationReadinessProjection {
  ...
  return {
    artifactKind: "delay_concentration_readiness_projection",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
