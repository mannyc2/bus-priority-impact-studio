...
// Reviewed-gold labels for the `permit_correlated_slowdown` detector (ADR-0018 steps 3-5).
// Wave 4 family adaptation (associational context). Standard 5-bucket frontend-use vocabulary applies,
// but `primary_finding` is rare by design — permit touches are broad street-work context, not causal
// evidence — so the expected label mass is `route_context`/`suppress` and the eval lens is leakage
// INTO findings (the `primaryFindingCount` should stay near zero), not primary survival. Calibration
// tags emphasize route-LION fanout, low match weight, temporal alignment, and the "not a causal
// attribution" caveat. The projection buckets routes into the shared readiness vocabulary.

export type PermitCorrelatedSlowdownReviewedFrontendUse =
  | "primary_finding"
  | "route_context"
  | "reviewer_only"
  | "needs_more_evidence"
...
export type PermitCorrelatedSlowdownCalibrationTag =
  | "true_permit_correlation_context"
  | "slow_route_supported"
  | "high_confidence_permit_touches"
  | "high_route_fanout"
  | "low_match_weight"
  | "temporal_misalignment"
  | "unrelated_work_type"
  | "broad_street_context_not_specific"
  | "near_threshold"
  | "not_a_causal_attribution"
  | "not_actionable_as_claim";

export type PermitCorrelatedSlowdownReviewedDecision = {
  readonly detectorId: string;
  readonly scopeId: string;
  readonly routeId: string | null;
  readonly sourceQueue: "candidate_review" | "skipped_control" | "clean_no_hit_control";
  readonly expectedFrontendUse: PermitCorrelatedSlowdownReviewedFrontendUse;
  readonly calibrationTags: readonly PermitCorrelatedSlowdownCalibrationTag[];
...
export type PermitCorrelatedSlowdownGoldLabel = PermitCorrelatedSlowdownReviewedDecision & {
...
export type PermitCorrelatedSlowdownReviewedGoldArtifact = {
  readonly artifactKind: "permit_correlated_slowdown_reviewed_gold";
  ...
  readonly labels: readonly PermitCorrelatedSlowdownGoldLabel[];
};

export type PermitCorrelatedSlowdownCandidateLike = {
...
export type PermitCorrelatedSlowdownCoverageLike = {
  readonly detectorId?: unknown;
  readonly routeId?: unknown;
  readonly scopeId?: unknown;
...
export type PermitCorrelatedSlowdownReviewedGoldEvaluation = {
  readonly artifactKind: "permit_correlated_slowdown_reviewed_gold_evaluation";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly summary: {
    ...
    // associational context: findings leakage = candidates labeled primary_finding (should be rare)
    readonly findingsLeakageCount: number;
  ...
  readonly byExpectedFrontendUse: Record<
    PermitCorrelatedSlowdownReviewedFrontendUse,
  ...
  readonly byReviewDepth: Record<
    ...
    { readonly expected: number; readonly emitted: number; readonly dropped: number }
  >;
  readonly emittedReviewed: readonly PermitCorrelatedSlowdownGoldLabel[];
  readonly droppedReviewed: readonly PermitCorrelatedSlowdownGoldLabel[];
  readonly unreviewedEmittedCandidates: readonly PermitCorrelatedSlowdownCandidateLike[];
};

export type PermitCorrelatedSlowdownReadinessProjection = {
  readonly artifactKind: "permit_correlated_slowdown_readiness_projection";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly items: readonly {
    ...
    readonly reviewedFrontendUse: PermitCorrelatedSlowdownReviewedFrontendUse | "unreviewed";
    readonly emittedCandidate: boolean;
    readonly reasonCode: string | null;
    readonly calibrationTags: readonly PermitCorrelatedSlowdownCalibrationTag[];
    readonly label: PermitCorrelatedSlowdownGoldLabel | null;
    readonly candidate: PermitCorrelatedSlowdownCandidateLike | null;
  }[];
};

export type BuildPermitCorrelatedSlowdownReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly reviewQueuePath: string;
  readonly decisionsPath: string;
  readonly decisions: readonly PermitCorrelatedSlowdownReviewedDecision[];
};

export type EvaluatePermitCorrelatedSlowdownReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly gold: PermitCorrelatedSlowdownReviewedGoldArtifact;
  readonly candidates: readonly PermitCorrelatedSlowdownCandidateLike[];
};

export type BuildPermitCorrelatedSlowdownReadinessProjectionInput =
  EvaluatePermitCorrelatedSlowdownReviewedGoldInput & {
    readonly coverage: readonly PermitCorrelatedSlowdownCoverageLike[];
  };
...
export function buildPermitCorrelatedSlowdownReviewedGoldArtifact(
  input: BuildPermitCorrelatedSlowdownReviewedGoldInput,
): PermitCorrelatedSlowdownReviewedGoldArtifact {
  const byCalibrationTag = new Map<PermitCorrelatedSlowdownCalibrationTag, number>();
  const byReviewDepth = new Map<string, number>();
  const labels = input.decisions
    .map((decision): PermitCorrelatedSlowdownGoldLabel => {
  ...
  return {
    artifactKind: "permit_correlated_slowdown_reviewed_gold",
    schemaVersion: 1,
...
export function evaluatePermitCorrelatedSlowdownReviewedGold(
  input: EvaluatePermitCorrelatedSlowdownReviewedGoldInput,
): PermitCorrelatedSlowdownReviewedGoldEvaluation {
  ...
  const byCalibrationTag = new Map<
    PermitCorrelatedSlowdownCalibrationTag,
    { expected: number; emitted: number; dropped: number }
  >();
  ...
  return {
    artifactKind: "permit_correlated_slowdown_reviewed_gold_evaluation",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
    summary: {
      ...
      primaryDroppedCount: primaryExpected.filter((label) => !emittedKeys.has(label.identityKey))
        .length,
      findingsLeakageCount: primaryExpected.length,
...
export function buildPermitCorrelatedSlowdownReadinessProjection(
  input: BuildPermitCorrelatedSlowdownReadinessProjectionInput,
): PermitCorrelatedSlowdownReadinessProjection {
  ...
  return {
    artifactKind: "permit_correlated_slowdown_readiness_projection",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
