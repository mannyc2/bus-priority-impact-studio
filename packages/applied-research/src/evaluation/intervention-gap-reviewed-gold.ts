...
// Reviewed-gold labels for the `intervention_gap` detector (ADR-0018 steps 3-5).
// Wave 3 intervention family. Standard 5-bucket frontend-use vocabulary applies. The claim is a
// scope-review candidate ("high pain, no strong dated treatment evidence"), only as honest as the
// treatment inventory, so calibration tags emphasize that absent/thin evidence is NOT proof of
// absence (a future/undated treatment can hide one) and the pain-threshold borough-fairness lens. The
// eval reports suppress leakage + reviewed-primary survival; the projection buckets routes into the
// shared readiness vocabulary.

export type InterventionGapReviewedFrontendUse =
  | "primary_finding"
  | "route_context"
  | "reviewer_only"
  | "needs_more_evidence"
...
export type InterventionGapCalibrationTag =
  | "true_intervention_gap"
  | "absent_evidence"
  | "thin_source_gap_evidence"
  | "high_pain_supported"
  | "treatment_inventory_incomplete"
  | "future_or_undated_treatment_possible"
  | "pain_threshold_borough_fairness"
  | "single_route_context"
  | "near_threshold"
  | "not_proof_of_absence"
  | "not_actionable_as_claim";

export type InterventionGapReviewedDecision = {
  readonly detectorId: string;
  readonly scopeId: string;
  readonly routeId: string | null;
  readonly sourceQueue: "candidate_review" | "skipped_control" | "clean_no_hit_control";
  readonly expectedFrontendUse: InterventionGapReviewedFrontendUse;
  readonly calibrationTags: readonly InterventionGapCalibrationTag[];
...
export type InterventionGapGoldLabel = InterventionGapReviewedDecision & {
...
export type InterventionGapReviewedGoldArtifact = {
  readonly artifactKind: "intervention_gap_reviewed_gold";
  ...
  readonly labels: readonly InterventionGapGoldLabel[];
};

export type InterventionGapCandidateLike = {
...
export type InterventionGapCoverageLike = {
  readonly detectorId?: unknown;
  readonly routeId?: unknown;
  readonly scopeId?: unknown;
...
export type InterventionGapReviewedGoldEvaluation = {
  readonly artifactKind: "intervention_gap_reviewed_gold_evaluation";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly byExpectedFrontendUse: Record<
    InterventionGapReviewedFrontendUse,
  ...
  readonly byReviewDepth: Record<
    ...
    { readonly expected: number; readonly emitted: number; readonly dropped: number }
  >;
  readonly emittedReviewed: readonly InterventionGapGoldLabel[];
  readonly droppedReviewed: readonly InterventionGapGoldLabel[];
  readonly unreviewedEmittedCandidates: readonly InterventionGapCandidateLike[];
};

export type InterventionGapReadinessProjection = {
  readonly artifactKind: "intervention_gap_readiness_projection";
  readonly schemaVersion: 1;
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  ...
  readonly items: readonly {
    ...
    readonly reviewedFrontendUse: InterventionGapReviewedFrontendUse | "unreviewed";
    readonly emittedCandidate: boolean;
    readonly reasonCode: string | null;
    readonly calibrationTags: readonly InterventionGapCalibrationTag[];
    readonly label: InterventionGapGoldLabel | null;
    readonly candidate: InterventionGapCandidateLike | null;
  }[];
};

export type BuildInterventionGapReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly reviewQueuePath: string;
  readonly decisionsPath: string;
  readonly decisions: readonly InterventionGapReviewedDecision[];
};

export type EvaluateInterventionGapReviewedGoldInput = {
  readonly generatedAt: string;
  readonly releaseMonth: string;
  readonly asOfMonth: string;
  readonly gold: InterventionGapReviewedGoldArtifact;
  readonly candidates: readonly InterventionGapCandidateLike[];
};

export type BuildInterventionGapReadinessProjectionInput =
  EvaluateInterventionGapReviewedGoldInput & {
    readonly coverage: readonly InterventionGapCoverageLike[];
  };
...
export function buildInterventionGapReviewedGoldArtifact(
  input: BuildInterventionGapReviewedGoldInput,
): InterventionGapReviewedGoldArtifact {
  const byCalibrationTag = new Map<InterventionGapCalibrationTag, number>();
  const byReviewDepth = new Map<string, number>();
  const labels = input.decisions
    .map((decision): InterventionGapGoldLabel => {
  ...
  return {
    artifactKind: "intervention_gap_reviewed_gold",
    schemaVersion: 1,
...
export function evaluateInterventionGapReviewedGold(
  input: EvaluateInterventionGapReviewedGoldInput,
): InterventionGapReviewedGoldEvaluation {
  ...
  return {
    artifactKind: "intervention_gap_reviewed_gold_evaluation",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
...
export function buildInterventionGapReadinessProjection(
  input: BuildInterventionGapReadinessProjectionInput,
): InterventionGapReadinessProjection {
  ...
  return {
    artifactKind: "intervention_gap_readiness_projection",
    schemaVersion: 1,
    generatedAt: input.generatedAt,
    releaseMonth: input.releaseMonth,
    asOfMonth: input.asOfMonth,
