...
import { dataProductJsonSemanticReasons, dataProductScoreVectorRouteIds } from "../data-products";
...
export async function evaluateDataProductScoreVectorRoutesCheck(input: {
  ...
  const parsed = existsSync(artifactPath)
    ? dataProductScoreVectorRouteIds(await Bun.file(artifactPath).json(), {
  ...
  const semanticReasons = [
    ...
    ...(parsed.wrongMonthRoutes.length > 0
      ...
      : []),
    ...(parsed.wrongRunRoutes.length > 0
      ? [`wrong_run_id_rows:${parsed.wrongRunRoutes.length}`]
      : []),
  ];
