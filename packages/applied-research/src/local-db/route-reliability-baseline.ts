import type { LocalPipelineDb, LocalRouteScheduleTimepoint } from "@bp/db/local";
import {
...
export type RouteReliabilityBaselineRow = {
...
export type RouteReliabilityBaselineResult = {
  isoMonth: string;
  routeCount: number;
  headwaySampleCount: number;
};
...
export function buildHeadwayGroups(
  routeId: string,
  ...
  const rows = await Promise.all(
    builtRoutes.map(async (route) =>
      routeBaseline(
        route.routeId,
        month,
        await listRouteSchedules(inputs.local.db, route.routeId, month),
      ),
    ),
  );
  await replaceRouteReliabilityRows(inputs.local.db, month, {
