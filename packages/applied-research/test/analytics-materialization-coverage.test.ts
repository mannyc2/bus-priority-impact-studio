...
function fixtureRegistryProducts(): MaterializationCoverageRegistryProduct[] {
  return MATERIALIZATION_COVERAGE_REGISTRY_PRODUCT_IDS.map((id) => ({
    id,
    label: id,
    expectedBasis: `${id} expected route universe`,
  }));
}

describe("analytics materialization coverage", () => {
