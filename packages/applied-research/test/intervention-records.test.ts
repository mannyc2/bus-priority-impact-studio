...
// @bp/applied-research/intervention-records. Bucketing/orchestration cases stay
...
describe("repairInterventionRecordsAliases (Fix 6)", () => {
  ...
  test("demotes invented enum values to custom labels (Fix P1.5, audit follow-up)", async () => {
    ...
    const { repairInvalidEnumValues } = await import(
      "@bp/applied-research/intervention-records"
    );
    const toolArgs = {
      sourceId: "x",
      interventionRecords: [
  ...
  test("filters null array elements (Fix P1.4 audit follow-up)", () => {
    ...
    expect(root.interventionRecords[0]!.customTreatments).toEqual(["frequency_increase", "other"]);
  });
