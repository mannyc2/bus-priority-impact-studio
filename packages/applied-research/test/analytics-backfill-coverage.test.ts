...
import { analyticsBackfillCoveragePath } from "../src/artifacts";
import { buildAnalyticsBackfillCoverageAudit } from "../src/evaluation";
import { loadAnalyticsBackfillCoverageLocalDbRows } from "../src/local-db";
...
describe("analytics backfill coverage", () => {
  test("reports missing and thin months for release-only backfill surfaces", () => {
    const db = createDb();
    try {
      ...
      expect(artifactPath).toBe(
        "/tmp/artifacts/analytics-backfill-coverage/2026-01_to_2026-03/coverage.json",
      );
      ...
      const ridership = audit.surfaces.find(
        (surface) => surface.surfaceId === "route_hourly_ridership",
      );
      ...
      const segmentSpeed = audit.surfaces.find(
        (surface) => surface.surfaceId === "route_segment_speed",
      );
